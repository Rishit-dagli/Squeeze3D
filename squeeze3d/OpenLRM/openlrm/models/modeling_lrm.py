...
class ModelLRM(nn.Module):
    ...
    def __init__(
        ...
        self.upsampler = nn.ConvTranspose2d(
            transformer_dim, triplane_dim, kernel_size=2, stride=2, padding=0
        )
    ...
    def forward(
        ...
        # render_region_size: int
        assert (
            image.shape[0] == source_camera.shape[0]
        ), "Batch size mismatch for image and source_camera"
        assert (
            image.shape[0] == render_cameras.shape[0]
        ), "Batch size mismatch for image and render_cameras"
        assert (
            image.shape[0] == render_anchors.shape[0]
        ), "Batch size mismatch for image and render_anchors"
        assert (
        ...
        assert (
            render_results["images_rgb"].shape[0] == N

...
from squeeze3d.OpenLRM.openlrm.utils.profiler import DummyProfiler
...
class LRMTrainer(Trainer):
    def __init__(self):
        ...
        self.pixel_loss_fn, self.perceptual_loss_fn, self.tv_loss_fn = (
    ...
    def _build_model(self, cfg):
        ...
        from squeeze3d.OpenLRM.openlrm.models import ModelLRM
    ...
    def _build_scheduler(self, optimizer, cfg):
        ...
        if cfg.train.scheduler.type == "cosine":
            from squeeze3d.OpenLRM.openlrm.utils.scheduler import (
    ...
    def _build_dataloader(self, cfg):
        # dataset class
        from squeeze3d.OpenLRM.openlrm.datasets import MixerDataset
    ...
    def _build_loss_fn(self, cfg):
        from squeeze3d.OpenLRM.openlrm.losses import LPIPSLoss, PixelLoss, TVLoss
    ...
    def forward_loss_local_step(self, data):
        ...
        if self.cfg.train.loss.perceptual_weight > 0.0:
            loss_perceptual = self.perceptual_loss_fn(
                outputs["images_rgb"], render_image
            )
    ...
    def train_epoch(
        ...
        for data in loader:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                # forward to loss
                outs, loss, loss_pixel, loss_perceptual, loss_tv = (
            ...
            if self.accelerator.sync_gradients:
                ...
                if (
                    ...
                    self.log_image_monitor(
                        ...
                        split="train",
                        renders=outs["images_rgb"]
                        .detach()[: self.cfg.logger.image_monitor.samples_per_log]
                        .cpu(),
                        gts=data["render_image"][
                            : self.cfg.logger.image_monitor.samples_per_log
                        ].cpu(),
                    )
        ...
        epoch_loss, epoch_loss_pixel, epoch_loss_perceptual, epoch_loss_tv = (
            epoch_losses.unbind()
        )
        epoch_loss_dict = {
    ...
    def evaluate(self, epoch: int = None):
        ...
        total_loss, total_loss_pixel, total_loss_perceptual, total_loss_tv = (
            total_losses.unbind()
        )
        total_loss_dict = {
        ...
        if epoch is not None:
            ...
            self.log_image_monitor(
                epoch=epoch,
                split="val",
                renders=sample_outs["images_rgb"][
                    : self.cfg.logger.image_monitor.samples_per_log
                ].cpu(),
                gts=sample_data["render_image"][
                    : self.cfg.logger.image_monitor.samples_per_log
                ].cpu(),
            )
        else:

...
from squeeze3d.OpenLRM.openlrm.datasets.cam_utils import (
...
from squeeze3d.OpenLRM.openlrm.utils.hf_hub import wrap_model_hub
from squeeze3d.OpenLRM.openlrm.utils.logging import configure_logger
from squeeze3d.OpenLRM.openlrm.utils.video import images_to_video
...
def parse_configs():
    ...
    if not os.environ.get("APP_ENABLED", None):
        ...
        assert (
            cfg.export_video or cfg.export_mesh
...
class LRMInferrer(Inferrer):
    ...
    def _build_model(self, cfg):
        from squeeze3d.OpenLRM.openlrm.models import model_dict
    ...
    def infer_planes(self, image: torch.Tensor, source_cam_dist: float):
        N = image.shape[0]
        source_camera = self._default_source_camera(
            dist_to_center=source_cam_dist, batch_size=N, device=self.device
        )
    ...
    def infer_mesh(
        ...
        vtx, faces = mcubes.marching_cubes(
            grid_out["sigma"].squeeze(0).squeeze(-1).cpu().numpy(), mesh_thres
        )
        vtx = vtx / (mesh_size - 1) * 2 - 1

        vtx_tensor = torch.tensor(
            vtx, dtype=torch.float32, device=self.device
        ).unsqueeze(0)
        vtx_colors = (
            self.model.synthesizer.forward_points(planes, vtx_tensor)["rgb"]
            .squeeze(0)
            .cpu()
            .numpy()
        )  # (0, 1)
        ...
        mesh.export(dump_mesh_path, file_type="obj")

    def encode(self, image_path: Image, source_cam_dist: float = 2.0):
        ...
        source_cam_dist = (
            self.cfg.source_cam_dist if source_cam_dist is None else source_cam_dist
        )

        # prepare image: [1, C_img, H_img, W_img], 0-1 scale
        image = torch.from_numpy(np.array(image_path)).to(self.device)
        image = image.permute(2, 0, 1).unsqueeze(0) / 255.0
        if image.shape[1] == 4:  # RGBA
            image = image[:, :3, ...] * image[:, 3:, ...] + (1 - image[:, 3:, ...])
        image = torch.nn.functional.interpolate(
            image, size=(source_size, source_size), mode="bicubic", align_corners=True
        )

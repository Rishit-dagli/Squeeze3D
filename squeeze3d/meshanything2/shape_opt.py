...
class ShapeOPTDecoder(OPTDecoder):
    ...
    def __init__(self, config: ShapeOPTConfig):
        ...
        self.token_embed_positions = OPTLoopEmbedding(
            10, config.word_embed_proj_dim, self.n_discrete_size
...
class OPTLoopEmbedding(nn.Embedding):
    ...
    def forward(
        ...
        for cur_batch_index in range(batch_size):
            ...
            idx_in_extra = torch.isin(
                cur_ids, torch.LongTensor([0, 1, 2]).to(input_ids.device)
            )
            if idx_in_extra:
                ...
                if cur_ids == self.n_discrete_size:
                    ...
                    if self.state[cur_batch_index] == 0:
                        face_ids[cur_batch_index] = (
                            7 + self.loop_state[cur_batch_index] % 3
                        )
                    else:
                        self.state[cur_batch_index] -= 1
                        face_ids[cur_batch_index] = (
                            4 + self.loop_state[cur_batch_index] % 3
                        )
    ...
    def init_state(self, template_tensor):
        ...
        self.state = torch.zeros(
            (batch_size, 1), dtype=torch.long, device=template_tensor.device
        )
        self.state[...] = 9
        self.loop_state = torch.zeros(
            (batch_size, 1), dtype=torch.long, device=template_tensor.device
        )

...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train and eval the NeRF RPN baseline."
    )
    ...
    parser.add_argument(
        "--dataset_split", default="", help="Path to the dataset split file."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load_backbone_only", action="store_true", help="Only load the backbone."
    )
    parser.add_argument(
        "--backbone_type",
        type=str,
    ...
    parser.add_argument(
        "--freeze_backbone", action="store_true", help="Freeze the backbone."
    )
    ...
    parser.add_argument(
        "--train_csv",
        default="",
        help="Path to the train csv. Only used if dataset_name is general.",
    )
    parser.add_argument(
        "--val_csv",
        default="",
        help="Path to the val csv. Only used if dataset_name is general.",
    )
    parser.add_argument(
        "--test_csv",
        default="",
        help="Path to the test csv. Only used if dataset_name is general.",
    )
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="The probability of extra rotation and scaling.",
    )
    ...
    parser.add_argument(
        "--rpn_nms_thresh", default=0.3, type=float, help="The NMS threshold."
    )
    parser.add_argument(
        "--rpn_fg_iou_thresh",
        default=0.35,
    ...
    parser.add_argument(
        "--rpn_score_thresh", default=0.0, type=float, help="The score threshold."
    )
    parser.add_argument(
        "--reg_loss_type",
        choices=["smooth_l1", "iou", "linear_iou", "giou", "diou"],
        default="smooth_l1",
        help="The type of regression loss to use for the RPN.",
    )
    ...
    parser.add_argument(
        "--save_results_path", default="", help="The path to save features"
    )
    parser.add_argument(
        "--output_all",
        action="store_true",
...
class Trainer:
    def __init__(self, args, rank=0, world_size=1, device_id=None, logger=None):
        ...
        if args.dataset_name == "hypersim":
            ...
            raise ValueError(
                "Dataset name {} is not supported".format(args.dataset_name)
            )
        ...
        self.anchor_generator = AnchorGenerator3D(
            anchor_sizes, aspect_ratios, is_normalized=normalize_aspect_ratios
        )
        ...
        self.rpn_head = RPNHead(
            ...
            self.args.rpn_head_conv_depth,
            rotate=self.args.rotated_bbox,
        )
        ...
        if args.check_arch:
            print(
                "Checking model architecture on GPU... (will exit after printing torchinfo summary)"
            )
    ...
    def init_datasets(self):
        if not self.args.dataset_split and not self.args.dataset_name == "general":
            raise ValueError(
                "The dataset split must be specified if not using general dataset."
            )
    ...
    def build_backbone(self):
        if self.args.backbone_type == "resnet":
            self.backbone = ResNet_FPN_256(
                Bottleneck, [3, 4, 6, 3], input_dim=4, is_max_pool=True
            )
    ...
    def train_loop(self):
        if self.args.dataset_name in ["hypersim", "front3d"]:
            ...
            self.train_set = GeneralRPNDataset(
                csv_path=self.args.train_csv,
                normalize_density=self.args.normalize_density,
            )
            self.val_set = GeneralRPNDataset(
                csv_path=self.args.val_csv,
                normalize_density=self.args.normalize_density,
            )
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            if i % self.args.log_interval == 0 and self.rank == 0:
                self.logger.info(
                    f"Epoch {epoch} [{i}/{len(self.train_loader)}] {scene_name}  "
                    f"Loss: {loss.item():.4f}  "
    ...
    def eval(self, dataset):
        ...
        for batch in tqdm(dataloader):
            ...
            output_list, losses, scores = self.model(
            ...
            if self.args.save_results:
                ...
                for i in range(len(scenes)):
                    ...
                    np.savez(
                        features_path + f"/{scenes[i]}.npz",
                        level_features=np.array(cur_features, dtype=object),
                        resolution=cur_res,
                    )
                    np.savez(
                        roi_path + f"/{scenes[i]}.npz",
                        proposals=cur_proposals,
                        level_indices=cur_level_indexes,
                    )
...
def main_worker(proc, nprocs, args, gpu_ids, init_method):
    ...
    dist.init_process_group(
        backend="nccl", init_method=init_method, world_size=nprocs, rank=proc
    )

...
def decode_fcos_obb(locations, box_regression):
    ...
    return torch.stack(
        [centers[:, 0], centers[:, 1], centers[:, 2], w, l, h, theta], dim=1
    )
...
def encode_fcos_obb(locations, boxes):
    ...
    assert (
        boxes.shape[0] == locations.shape[0]
    ), "number of boxes should be equal to number of locations"
...
def box_iou_3d(boxes1: Tensor, boxes2: Tensor) -> Tensor:
    ...
    """
    assert (
        boxes1.shape[1] == 6 or boxes1.shape[1] == 7
    ), "boxes should have 6 or 7 elements"
    assert (
        boxes1.shape[1] == boxes2.shape[1]
    ), "boxes1 and boxes2 should have the same number of elements"
    ...
    if boxes1.shape[1] == 6:
        ...
        iou = cal_iou_3d(boxes1_rep.cuda(), boxes2_rep.cuda()).to(
            dtype=torch.float32, device=boxes1.device
        )
...
def obb2points_3d(obboxes):
    ...
    vector = torch.cat(
        [w / 2 * Cos - l / 2 * Sin, w / 2 * Sin + l / 2 * Cos, h / 2], dim=-1
    )

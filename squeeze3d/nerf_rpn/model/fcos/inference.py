...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(
        ...
        box_regression = box_regression.view(N, box_reg_dim, W, H, D).permute(
            0, 2, 3, 4, 1
        )
    ...
    def forward(
        ...
        boxes_all = []
        scores_all = []
        for level, (l, o, b, c) in enumerate(
            zip(locations, box_cls, box_regression, centerness)
        ):

            masks_per_level = (
                padding_masks[level] if padding_masks is not None else None
            )
            boxes, scores = self.forward_for_single_feature_map(
                l, o, b, c, grid_sizes, masks_per_level
            )

            level_indinces = [
                torch.zeros(item.shape[:-1]).cuda() + level for item in boxes
            ]

            boxes = [
                torch.cat([level_indinces[ind][..., None], boxes[ind]], dim=-1)
    ...
    def select_over_all_levels(self, boxes, scores):
        ...
        for i in range(num_scenes):
            ...
            boxes_results.append(
                torch.cat([box_indices_keep[..., None], boxes_keep], dim=-1)
            )

...
class IOULoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None):
        ...
        d_intersect = torch.min(pred_front, target_front) + torch.min(
            pred_back, target_back
        )
        g_d_intersect = torch.max(pred_front, target_front) + torch.max(
            pred_back, target_back
        )
    ...
    def forward(self, pred, target, weight=None):
        ...
        if self.loss_type == "iou" or self.loss_type == "linear_iou":
            ious, _, _, _, unions = cal_iou_3d(
                pred_boxes.unsqueeze(0), target_boxes.unsqueeze(0), verbose=True
            )
...
class FCOSLossComputation(object):
    ...
    def __init__(
        ...
        if self.iou_loss_type != "smooth_l1":
            self.box_reg_loss_func = (
                IOULoss(self.iou_loss_type)
                if not use_obb
                else RotatedIOULoss(self.iou_loss_type)
            )
        else:
    ...
    def get_sample_region(
        self, gt, strides, num_points_per, gt_xs, gt_ys, gt_zs, radius=1.0
    ):
        """
        This code is from
        ...
        for level, n_p in enumerate(num_points_per):
            ...
            center_gt[beg:end, :, 3] = torch.where(
                xmax > gt[beg:end, :, 3], gt[beg:end, :, 3], xmax
            )
            center_gt[beg:end, :, 4] = torch.where(
                ymax > gt[beg:end, :, 4], gt[beg:end, :, 4], ymax
            )
            center_gt[beg:end, :, 5] = torch.where(
                zmax > gt[beg:end, :, 5], gt[beg:end, :, 5], zmax
            )
            beg = end
    ...
    def prepare_targets(self, points, targets):
        object_sizes_of_interest = [[-1, 16], [16, 32], [32, 64], [64, INF]]
        ...
        for level in range(len(points)):
            ...
            reg_targets_per_level = torch.cat(
                [reg_targets_per_im[level] for reg_targets_per_im in reg_targets], dim=0
            )

            if self.norm_reg_targets:
                reg_targets_per_level[..., :6] = (
                    reg_targets_per_level[..., :6] / self.fpn_strides[level]
                )
            reg_targets_level_first.append(reg_targets_per_level)

        return labels_level_first, reg_targets_level_first

    def compute_targets_for_locations_obb(
        ...
        for bboxes in targets:
            ...
            if bboxes.shape[0] == 0:
                ...
                reg_targets.append(
                    torch.zeros((locations.shape[0], 8), device=locations.device)
                )
                continue
            ...
            targets_per_loc = [
                encode_fcos_obb(
                    locations, bboxes[i : i + 1, :].expand(locations.shape[0], -1)
                )
    ...
    def compute_targets_for_locations(
        ...
        for bboxes in targets:
            ...
            if bboxes.shape[0] == 0:
                labels.append(torch.zeros(locations.shape[0], device=locations.device))
                reg_targets.append(
                    torch.zeros((locations.shape[0], 6), device=locations.device)
                )
                continue
    ...
    def compute_2d_projection_loss(self, box_reg, reg_targets, weights):
        ...
        pred_boxes = torch.cat(
            [pred_boxes, torch.ones(pred_boxes.shape[0], 1, device=box_reg.device)],
            dim=1,
        )
        target_boxes = torch.cat(
            [target_boxes, torch.ones(target_boxes.shape[0], 1, device=box_reg.device)],
            dim=1,
        )
        ...
        box_reg_loss_2d = (
            F.smooth_l1_loss(
                pred_boxes_2d, target_boxes_2d, beta=1 / 9, reduction="none"
            )
            / 160
        )
    ...
    def __call__(
        ...
        for l in range(len(labels)):
            box_cls_flatten.append(
                box_cls[l].permute(0, 2, 3, 4, 1).reshape(-1)
            )  # binary classification
            box_regression_flatten.append(
                box_regression[l].permute(0, 2, 3, 4, 1).reshape(-1, box_reg_dim)
            )
        ...
        cls_loss = (
            self.cls_loss_func(box_cls_flatten, labels_flatten, reduction="sum")
            / num_pos_avg_per_gpu
        )
        ...
        if pos_inds.numel() > 0:
            ...
            sum_centerness_targets_avg_per_gpu = self.reduce_sum(
            ...
            if (
                self.use_obb
                and self.use_additional_l1_loss
                and self.iou_loss_type != "smooth_l1"
            ):
                ...
                additional_l1_loss = (
                    additional_l1_loss.sum() / sum_centerness_targets_avg_per_gpu
                )
                reg_loss += additional_l1_loss

            if self.use_obb and self.proj2d_loss_weight > 0:
                proj2d_loss = (
                    self.compute_2d_projection_loss(
                        box_regression_flatten, reg_targets_flatten, centerness_targets
                    )
                    / sum_centerness_targets_avg_per_gpu
                )

...
def permute_and_flatten(
    layer: Tensor, N: int, A: int, C: int, W: int, H: int, D: int
) -> Tensor:
    """
...
def get_w2cs(res: int = 160):
    ...
    w2cs = [
        torch.Tensor(np.linalg.inv(c2w_from_loc_and_at(position, centroid)))
...
def project(
    ...
    box_coords_cam = torch.matmul(
        pose_mat, torch.transpose(box_coords, 0, 1).float()
    )  # [4x8]
...
def concat_box_prediction_layers(
    box_cls: List[Tensor], box_regression: List[Tensor], num_bbox_digits: int
):
    ...
    for box_cls_per_level, box_regression_per_level in zip(box_cls, box_regression):
        ...
        box_regression_per_level = permute_and_flatten(
            box_regression_per_level, N, A, num_bbox_digits, W, H, D
        )
    ...
    box_regression = torch.cat(box_regression_flattened, dim=1).reshape(
        -1, num_bbox_digits
    )
...
class RotatedIOULoss(nn.Module):
    ...
    def forward(self, pred, target, weight=None):
        if self.loss_type == "iou" or self.loss_type == "linear_iou":
            ious, _, _, _, unions = cal_iou_3d(
                pred.unsqueeze(0), target.unsqueeze(0), verbose=True
            )
...
class RegionProposalNetwork(nn.Module):
    def __init__(
        ...
        self.fg_bg_sampler = BalancedPositiveNegativeSampler(
            batch_size_per_mesh, positive_fraction
        )
    ...
    def get_padding_masks(self, meshes, features, ori_sizes):
        ...
        for masks_per_level in masks:
            ...
            masks_flattened.append(
                permute_and_flatten(masks_per_level, N, A, 1, W, H, D)
            )
    ...
    def assign_targets_to_anchors(
        ...
        for i, (anchors_per_mesh, gt_boxes) in enumerate(zip(anchors, targets)):
            if gt_boxes.numel() == 0:
                ...
                matched_gt_boxes_per_mesh = torch.zeros(
                    anchors_per_mesh.shape, dtype=torch.float32, device=device
                )
                labels_per_mesh = torch.zeros(
                    (anchors_per_mesh.shape[0],), dtype=torch.float32, device=device
                )
            else:
                if gt_boxes.size(1) == 7:
                    ...
                    match_quality_matrix = self.box_similarity(
                        gt_boxes_aabb, anchors_per_mesh, self.iou_batch_size
                    )
                else:
                    match_quality_matrix = self.box_similarity(
                        gt_boxes, anchors_per_mesh, self.iou_batch_size
                    )
    ...
    def filter_proposals(
        ...
        final_scores = []
        final_indexes = []
        for boxes, scores, indexes, lvl, mesh_shape in zip(
            proposals, objectness_prob, level_indexes, levels, mesh_shapes
        ):
            boxes = clip_boxes_to_mesh(boxes, mesh_shape)
            # remove small boxes
            keep = remove_small_boxes(boxes, self.min_size)
            boxes, scores, indexes, lvl = (
                boxes[keep],
                scores[keep],
                indexes[keep],
                lvl[keep],
    ...
    def compute_loss(
        ...
        if self.reg_loss_type == "smooth_l1":
            box_reg_loss_3d = F.smooth_l1_loss(
            ...
            box_reg_loss_3d = self.rotated_iou_loss(
                pred_bbox[sampled_pos_inds], target_bbox_cat[sampled_pos_inds]
            ) / (sampled_inds.numel())
        ...
        if target_bbox.size(1) == 6:
            ...
            pred_bbox = torch.cat(
                [pred_bbox[sampled_pos_inds, :3], pred_bbox[sampled_pos_inds, 3:]],
                dim=0,
            )
            target_bbox = torch.cat(
                [target_bbox[sampled_pos_inds, :3], target_bbox[sampled_pos_inds, 3:]],
                dim=0,
            )
        ...
        if torch.cuda.is_available():
            pred_bbox = torch.cat(
                [pred_bbox, torch.ones(pred_bbox.shape[0], 1).cuda()], dim=1
            )
            target_bbox = torch.cat(
                [target_bbox, torch.ones(target_bbox.shape[0], 1).cuda()], dim=1
            )
        else:
            pred_bbox = torch.cat([pred_bbox, torch.ones(pred_bbox.shape[0], 1)], dim=1)
            target_bbox = torch.cat(
                [target_bbox, torch.ones(target_bbox.shape[0], 1)], dim=1
            )
        ...
        box_reg_loss_2d = (
            F.smooth_l1_loss(pred_bbox_2d, target_bbox_2d, beta=1 / 9, reduction="sum")
    ...
    def forward(
        ...
        if objectness_output_paths is not None:
            self.output_objectness(
                objectness, original_mesh_sizes, objectness_output_paths
            )
        ...
        num_anchors_per_level = [
            s[0] * s[1] * s[2] * s[3] for s in num_anchors_per_level_shape_tensors
        ]

        objectness, pred_bbox_deltas, pred_bbox_deltas_unflattened = (
        ...
        padding_masks = (
            self.get_padding_masks(meshes, features, original_mesh_sizes)
            if num_meshes > 1
            else None
        )
        ...
        if not self.training:
            proposals = self.box_coder.decode_list(
                pred_bbox_deltas_unflattened, non_cat_anchors
            )
            level_indexes = proposals[..., -1]
            proposals = proposals[..., : self.num_bbox_digits]
            boxes, scores, level_indexes = self.filter_proposals(
                proposals,
                objectness,
                level_indexes,
                mesh_sizes,

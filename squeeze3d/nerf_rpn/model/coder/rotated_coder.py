...
class RotatedCoder(BaseBBoxCoder):
    ...
    def encode_single(self, gt_rois: Tensor, ex_drois: Tensor):
        ...
        targets_dx = (
            torch.cos(ex_drois[:, 6]) * coord[:, 0]
            + torch.sin(ex_drois[:, 6]) * coord[:, 1]
        ) / ex_widths
        targets_dy = (
            -torch.sin(ex_drois[:, 6]) * coord[:, 0]
            + torch.cos(ex_drois[:, 6]) * coord[:, 1]
        ) / ex_heights
    ...
    def decode_single(self, deltas: Tensor, ex_drois: Tensor):
        ...
        pred_ctr_x = (
            dx * widths[:, None] * torch.cos(angles[:, None])
            - dy * heights[:, None] * torch.sin(angles[:, None])
            + ctr_x[:, None]
        )
        pred_ctr_y = (
            dx * widths[:, None] * torch.sin(angles[:, None])
            + dy * heights[:, None] * torch.cos(angles[:, None])
            + ctr_y[:, None]
        )
        ...
        pred_angle[pred_angle > torch.pi / 2] = (
            pred_angle[pred_angle > torch.pi / 2] - torch.pi
        )

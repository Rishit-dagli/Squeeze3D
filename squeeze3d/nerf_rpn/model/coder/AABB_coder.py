...
def encode_boxes_3d(reference_boxes: Tensor, proposals: Tensor) -> Tensor:
    ...
    targets = torch.cat(
        (targets_dx, targets_dy, targets_dz, targets_dw, targets_dh, targets_dd), dim=1
    )
    return targets
...
class AABBCoder(BaseBBoxCoder):
    ...
    def __init__(self, bbox_xform_clip: float = math.log(2000.0)) -> None:
        """
        Args:
    ...
    def decode_single(self, rel_codes: Tensor, boxes: Tensor) -> Tensor:
        ...
        c_to_c_d = (
            torch.tensor(0.5, dtype=pred_ctr_z.dtype, device=pred_d.device) * pred_d
        )

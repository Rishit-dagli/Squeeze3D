...
def cal_giou_3d(
    box3d1: torch.Tensor, box3d2: torch.Tensor, enclosing_type: str = "smallest"
):
    ...
    Returns:
        giou (torch.Tensor): (B, N) 3d GIoU loss
...
def cal_diou_3d(
    ...
    diou = 1.0 - iou3d + d2 / c2
    return diou, iou3d


def enclosing_box(
    corners1: torch.Tensor, corners2: torch.Tensor, enclosing_type: str = "smallest"
):
...
def enclosing_box_pca(corners1: torch.Tensor, corners2: torch.Tensor):
    ...
    w = (
        p1.max(dim=-1)[0] - p1.min(dim=-1)[0]
    )  # (B*N, ),  width of rotated enclosing box
    h = (
        p2.max(dim=-1)[0] - p2.min(dim=-1)[0]
...
def eigenvector_22(x: torch.Tensor):
    ...
    v1 = (a - b - delta) / 2.0 / c
    v1 = torch.stack(
        [v1, torch.ones_like(v1, dtype=torch.double, device=v1.device)], dim=-1
    )  # (..., 2)
    v2 = (a - b + delta) / 2.0 / c
    v2 = torch.stack(
        [v2, torch.ones_like(v2, dtype=torch.double, device=v2.device)], dim=-1
    )  # (..., 2)

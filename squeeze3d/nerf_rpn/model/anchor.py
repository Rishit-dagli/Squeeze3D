...
class AnchorGenerator3D(nn.Module):
    ...
    def generate_anchors(self, scales, xyz_ratios, dtype=torch.float32, device="cpu"):
        ...
        for ratio in xyz_ratios:
            ...
            if self.is_normalized:
                ...
                for i in range(3):
                    weight *= ratio[i]
                weight = torch.pow(weight, 1.0 / 3.0)
                P = P / weight
    ...
    def grid_anchors(self, grid_sizes, strides, dtype=torch.float32, device="cpu"):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, self.cell_anchors):
            ...
            shifts_x = (
                torch.arange(0, grid_x, dtype=torch.float32, device=device) * stride_x
            )
            shifts_y = (
                torch.arange(0, grid_y, dtype=torch.float32, device=device) * stride_y
            )
            shifts_z = (
                torch.arange(0, grid_z, dtype=torch.float32, device=device) * stride_z
            )
            ...
            shifts = torch.stack(
                (shift_x, shift_y, shift_z, shift_x, shift_y, shift_z), dim=1
            )

            # For every (base anchor, output anchor) pair,
            # offset each zero-centered base anchor by the center of the output anchor.
            anchors.append(
                (shifts.view(-1, 1, 6) + base_anchors.view(1, -1, 6)).reshape(-1, 6)
            )
    ...
    def get_padding_masks(self, meshes, feature_maps, ori_sizes):
        ...
        strides = [
            [
                torch.tensor(mesh_size[0] // g[0], dtype=torch.int64, device=device),
                torch.tensor(mesh_size[1] // g[1], dtype=torch.int64, device=device),
                torch.tensor(mesh_size[2] // g[2], dtype=torch.int64, device=device),
            ]
            for g in grid_sizes
        ]
        ...
        for size, stride, num_anchors in zip(grid_sizes, strides, num_anchors_all):
            ...
            for ori_size in ori_sizes:
                ...
                limits = torch.ceil(ori_size / stride).to(
                    dtype=torch.int64, device=device
                )
                mask = torch.zeros(
                    (num_anchors, *size), dtype=torch.bool, device=device
                )
    ...
    def __init__(self, in_channels, num_anchors, conv_depth=1, rotate=False):
        ...
        if not rotate:
            self.bbox_pred = nn.Conv3d(
                in_channels, num_anchors * 6, kernel_size=1, stride=1
            )
        else:
            self.bbox_pred = nn.Conv3d(
                in_channels, num_anchors * 8, kernel_size=1, stride=1
            )

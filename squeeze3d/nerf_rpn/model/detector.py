...
class ProposalTargetLayer(nn.Module):
    ...
    def _sample_rois_pytorch(
        ...
        for i in range(batch_size):
            overlaps.append(
                batched_box_iou(all_rois[i][..., 1:], gt_boxes[i]).to(cur_device)
            )
            ...
            cur_gt_label = (
                gt_label[i]
                .repeat([cur_max_overlaps.size(0), 1])
                .to(cur_max_overlaps.get_device())
            )
        ...
        if is_sample:
            ...
            rois_batch = (
                all_rois[0]
                .new(batch_size, rois_per_image, all_rois[0].shape[-1])
                .zero_()
            )
            gt_rois_batch = (
                all_rois[0]
                .new(batch_size, rois_per_image, gt_boxes[0].shape[-1])
                .zero_()
            )
            ...
            for i in range(batch_size):
                ...
                gt_rois_batch.append(
                    all_rois[0].new(all_rois[i].size(0), gt_boxes[0].shape[-1]).zero_()
                )
...
class ROIPool(nn.Module):
    def __init__(
        self,
        output_size=[1, 1, 1],
        spatial_scale=[1, 1, 1, 1],
        enlarge_scale=0.2,
        is_rotated_bbox=False,
        feature_extracting_type="pooling",
        max_res=200,
        remap=False,
        use_cuda=False,
    ):
        ...
        for f, r in zip(features, rois):
            ...
            for l in range(self.canonical_level):
                ...
                if level_r.size(0) > 0:
                    ...
                    roi_features.append(
                        self.align(level_f, level_r, float(1 / self.spatial_scale[l]))
                    )
    ...
    def rotated_forward(self, features: List[List[Tensor]], rois: List[Tensor]):
        ...
        for i in range(batch_size):
            ...
            for level in range(len(features[i])):
                ...
                rois_grid_size = torch.ceil(
                    level_rois[:, 3:6] / spatial_scale[level]
                ).to(torch.long)
                ...
                grid_x, grid_y, grid_z = torch.meshgrid(
                    torch.arange(max_grid_size[0]),
                    torch.arange(max_grid_size[1]),
                    torch.arange(max_grid_size[2]),
                    indexing="ij",
                )
                ...
                rotation_mat = torch.stack(
                    [
                        torch.cat(
                            [torch.cos(theta), -torch.sin(theta), zeros_tensor], dim=1
                        ),
                        torch.cat(
                            [torch.sin(theta), torch.cos(theta), zeros_tensor], dim=1
                        ),
                ...
                position = position - (rois_grid_size[..., None] - 1) / 2.0
                ...
                all_grid_features = 0.0

                position_mask = (
                    ((position[0] >= 0) & (position[0] <= level_features_size[1] - 1))
                ...
                for a in range(len(eight_pts_func)):
                    interpolation_pts_list = [
                        eight_pts_func[a][0](position[0]),
                        eight_pts_func[a][1](position[1]),
                        eight_pts_func[a][2](position[2]),
                    ]
                    interpolation_pts = torch.stack(
                        [
                            torch.clip(
                                interpolation_pts_list[0],
                                min=0,
                                max=level_features_size[1] - 1,
                            ),
                            torch.clip(
                                interpolation_pts_list[1],
                                min=0,
                                max=level_features_size[2] - 1,
                            ),
                            torch.clip(
                                interpolation_pts_list[2],
                                min=0,
                                max=level_features_size[3] - 1,
                            ),
                        ]
                    ).to(torch.long)

                    interpolation_pts_feature = level_features[
                        :,
                        interpolation_pts[0],
                        interpolation_pts[1],
                        interpolation_pts[2],
                    ...
                    scale = (
                        (position[0] - interpolation_pts_list[0]).abs()
                    ...
                    all_grid_features = (
                        all_grid_features
                        + interpolation_pts_feature * (1.0 - scale[None, ...])
                    )

                all_grid_features = all_grid_features * position_mask[None, ...]
                all_grid_features = all_grid_features / len(eight_pts_func)

                all_grid_features = all_grid_features.reshape(
                    all_grid_features.shape[:1]
                    + position_shape[1:2]
                    + tuple(max_grid_size.tolist())
                )
                ...
                for roi_index in range(all_grid_features.size(0)):
                    ...
                    grid_feature = all_grid_features[roi_index][
                        :, : grid_size[0], : grid_size[1], : grid_size[2]
                    ]

                    if self.feature_extracting_type == "pooling":
                        kernel_size = torch.ceil(
                            grid_size / self.output_size.to(cur_device)
                        ).type(torch.int)
                        padding = (
                            kernel_size * self.output_size.to(cur_device) - grid_size
                        ).type(torch.int)
                        extracting_layer = nn.MaxPool3d(
                            kernel_size=kernel_size.tolist(),
                            stride=kernel_size.tolist(),
                        )

                        grid_feature = torch.nn.functional.pad(
                            grid_feature,
                            (
                                0,
                                int(padding[2]),
                        ...
                        extracted_feat = nn.functional.interpolate(
                            grid_feature[None, ...],
                            size=tuple(self.output_size.to(torch.long).tolist()),
                            mode="trilinear",
                            align_corners=True,
                        )
                    else:
    ...
    def normal_forward(self, features, rois):
        ...
        for i in range(batch_size):
            ...
            cur_spatial_scale = (
                spatial_scale.repeat([rois[i].size(0), 1])
                .gather(1, level_index)
                .to(rois[0].get_device())
            )
            ...
            for j in range(position.size(0)):
                ...
                kernel_size = torch.ceil(cur_feat_size / self.output_size).type(
                    torch.int
                )
                padding = (kernel_size * self.output_size - cur_feat_size).type(
                    torch.int
                )
                pooling_layer = nn.MaxPool3d(
                    kernel_size=kernel_size.tolist(), stride=kernel_size.tolist()
                )

                cur_feat = torch.nn.functional.pad(
                    cur_feat,
...
class RCNN(nn.Module):
    def __init__(
        ...
        if is_add_layer:
            ...
            for _ in range(2):
                convs.append(
                    nn.Conv3d(self.in_planes, self.in_planes, kernel_size=3, padding=1)
                )
        ...
        if is_flatten:
            ...
            self.RCNN_bbox_pred = nn.Linear(
                self.in_planes * self.flatten_size, self.reg_dim
            )
            self.RCNN_cls_score = nn.Linear(
                self.in_planes * self.flatten_size, self.n_classes
            )
        else:
...
class Classification_Model(nn.Module):
    ...
    def compute_loss(
        self, pred_scores, pred_bbox_deltas, gt_labels, regression_targets
    ):
        ...
        box_loss = (
            ...
            if inds.numel() != 0
            else torch.tensor(0.0, dtype=regression_targets.dtype).cuda()
        )
    ...
    def forward(
        self, rois, gt_bboxes, gt_bbox_labels, features, is_sample=True, is_reg=False
    ):
        ...
        if self.feature_extractor is not None:
            ...
            features = [
                [f[i].to(torch.float) for f in features]
                for i in range(len(features[0]))
            ]

        gt_labels, sample_rois, gt_bbox = self.sample_model(
            rois, gt_bboxes, gt_bbox_labels, is_sample=is_sample
        )
        ...
        for i in range(input_batch):
            ...
            pred_cls_prob_batch.append(
                cls_prob[roi_batch_split[i] : roi_batch_split[i + 1]]
            )
        ...
        for i in range(len(pred_bbox_deltas_batch)):
            proposals.append(
                self.box_coder.decode_single(
                    pred_bbox_deltas_batch[i], sample_rois_batch[i]
                )
            )
        ...
        if is_sample:
            regression_targets = self.box_coder.encode_single(
                gt_bbox.reshape(-1, gt_bbox.size(-1)),
                sample_rois[..., 1:].reshape(-1, sample_rois.size(-1) - 1),
            )
            regression_targets = regression_targets.reshape(
                input_batch, sample_batch, regression_targets.size(-1)
            )
...
if __name__ == "__main__":
    ...
    model = Classification_Model(sample_model, pooling_model, RCNN_model)
    roi = [
        [
            torch.cat(
                [torch.rand([20, 3]) * 50, torch.rand([20, 3]) * 127 + 128], dim=1
            )
            for _ in range(3)
        ]
    ]
    gt_box = [
        torch.cat([torch.rand([5, 3]) * 50, torch.rand([5, 3]) * 127 + 128], dim=1)
    ]
    gt_label = [torch.ones([5])]
    feature = [
        [
            torch.rand(512, 128, 128, 128),
            torch.rand(512, 64, 64, 64),
            torch.rand(512, 32, 32, 32),
        ]
    ]

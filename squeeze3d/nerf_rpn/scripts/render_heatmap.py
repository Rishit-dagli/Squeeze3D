...
def gkern_3d(w=10, l=10, h=3, sig=1.0):
    """
    ...
    """
    ax = np.linspace(-(w - 1) / 2.0, (w - 1) / 2.0, w)
    ay = np.linspace(-(l - 1) / 2.0, (l - 1) / 2.0, l)
    az = np.linspace(-(h - 1) / 2.0, (h - 1) / 2.0, h)
...
def load_alpha_and_proposals(
    feature_path: str, proposal_path: str, json_path: str, args
):
...
def visualize_3dgrid(heatmap, downsample=4):
    ...
    grid_y, grid_x, grid_z = np.meshgrid(
        np.arange(heatmap.shape[1]),
        np.arange(heatmap.shape[0]),
        np.arange(heatmap.shape[2]),
    )
    grid = np.concatenate(
        [
            grid_x.reshape(-1, 1),
    ...
    fig = px.scatter_3d(
        df, x="x", y="y", z="z", size="value", color="value", symbol="species"
    )
    fig.show()
...
def get_overview_position(
    room_bbox, res, downsample, in_offset=0.3, cam_height=2, index=2
):
    x1, y1, x2, y2 = (
        room_bbox[0] + in_offset,
        room_bbox[1] + in_offset,
        room_bbox[3] - in_offset,
        room_bbox[4] - in_offset,
    )
    focal_point = np.array([(x1 + x2) / 2.0, (y1 + y2) / 2.0, 1.0])
...
def render_volume(
    heatmap, room_bbox, res, output_dir, args, json_dict=None, boxes_8=None
):
    heatmap = heatmap[0 :: args.downsample, 0 :: args.downsample, 0 :: args.downsample]
    heatmap *= args.value_scale
    print(
        "mean={}, std={}, min={}, max={}".format(
            np.mean(heatmap), np.std(heatmap), np.min(heatmap), np.max(heatmap)
        )
    )
    ...
    if json_dict == None:  # debug
        focal_point, cam_position = get_overview_position(
            room_bbox, res, args.downsample, index=2
        )
        ...
        names, cam_positions, focal_points, poses = frame2config(
            frame_dict, room_bbox, res, args.downsample
        )
        fl_x, fl_y, cx, cy = (
            json_dict["fl_x"],
            json_dict["fl_y"],
        ...
        for name, cam_position, focal_point, pose in zip(
            names, cam_positions, focal_points, poses
        ):
            p.camera.position = cam_position
            p.camera.focal_point = focal_point
            if args.interactive:
                ...
                renderPM.drawToFile(
                    svg2rlg(join(output_dir, name + "_hmp.svg")),
                    join(output_dir, name + "_hmp.png"),
                    fmt="PNG",
                )
                ...
                if args.concat_img:
                    ...
                    img1 = cv2.imread(
                        join(
                            args.dataset_dir,
                            args.scene_name,
                            "val/screenshots/" + name + ".jpg",
                        )
                    )
                    shape = img1.shape
                    hmp = cv2.resize(
                        hmp, (shape[1], shape[0]), interpolation=cv2.INTER_AREA
                    )
                    img2 = copy.deepcopy(img1)
                    img2 = project_obb_to_image(
                        img2, intrinsic_mat, np.linalg.inv(pose), boxes_8
                    )
...
def parse_args():
    ...
    parser.add_argument(
        "--top_n", type=int, default=100, help="top n proposals to be used for heatmap."
    )
    ...
    parser.add_argument(
        "--value_scale", type=float, default=20, help="value scaling for heatmap"
    )
    parser.add_argument(
        "--downsample", type=int, default=2, help="downsample factor for heatmap"
    )
    parser.add_argument(
        "--gaussian_sigma", type=float, default=5, help="sigma for gaussian kernel"
    )
    parser.add_argument(
        "--concat_img", action="store_true", help="concatenate heatmap with NeRF image"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="interactive mode, used in .ipynb"
    )
...
if __name__ == "__main__":
    ...
    for scene_name in scene_list:
        ...
        assert os.path.isfile(feature_path), "feature file not found: {}".format(
            feature_path
        )
        assert os.path.isfile(proposal_path), "proposal file not found: {}".format(
            proposal_path
        )
        assert os.path.isfile(train_json_path), "train json file not found: {}".format(
            train_json_path
        )
        assert os.path.isfile(val_json_path), "val json file not found: {}".format(
            val_json_path
        )
        scene_output_dir = join(args.output_dir, scene_name)
        os.makedirs(scene_output_dir, exist_ok=True)

        alpha, proposals, room_bbox, res = load_alpha_and_proposals(
            feature_path, proposal_path, train_json_path, args
        )
        aabbs = obb2hbb(proposals).astype(int)
        boxes_point8 = grid2world(obb2point8(proposals), room_bbox, res)
        for i in range(3):
            aabbs[:, [i, i + 3]] = np.clip(
                aabbs[:, [i, i + 3]], a_min=0, a_max=res[i] - 1
            )
        ...
        render_volume(
            heatmap, room_bbox, res, scene_output_dir, args, val_json_dict, boxes_point8
        )

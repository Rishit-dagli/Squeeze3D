...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train and eval the NeRF RPN baseline on 3D FRONT."
    )
    ...
    parser.add_argument(
        "--save_path",
        default="",
        help="The path to save the model. It will create a folder named the process_name under save_root",
    )
    ...
    parser.add_argument(
        "--pretrained", default="", help="The path to the pretrained backbone to load."
    )

    parser.add_argument("--bash_file", default="", help="The bash to run the code.")
    parser.add_argument(
        "--fine_tune", action="store_true", help="Fine-tune the backbone."
    )
    parser.add_argument(
        "--backbone_type",
        type=str,
        default="resnet",
        choices=["resnet", "vgg_AF", "vgg_EF", "swin"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Input dimension for backbone.",
    )
    parser.add_argument(
        "--resolution",
        type=int,
    ...
    # Training parameters
    parser.add_argument(
        "--batch_size", default=2, type=int, help="The num of scenes in a batch."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.5,
        type=float,
        help="The probability of rotating the scene.",
    )
    parser.add_argument(
        "--flip_prob",
        default=0.5,
    ...
    parser.add_argument(
        ...
        default="wandb_root",
        type=str,
        help="The root of the process in wandb and model save.",
    )
    parser.add_argument(
        "--process_name",
        default="wandb_process",
        type=str,
        help="The name of the process.",
    )
    parser.add_argument(
        "--init_method",
        default="tcp://127.0.0.1:23441",
        type=str,
        help="init method of wandb.",
    )
    ...
    # cls parameters
    parser.add_argument(
        "--n_classes", default=2, type=int, help="The number of output classes."
    )
    ...
    parser.add_argument(
        "--remap", action="store_true", help="Use FPN version of alignment mapping."
    )
    parser.add_argument(
        "--is_add_layer",
        action="store_true",
    ...
    parser.add_argument(
        "--feature_extracting_type",
        default="pooling",
        choices=["pooling", "interpolation"],
    )

    parser.add_argument(
        "--nms_thresh", default=0.1, type=float, help="The NMS threshold."
    )
    parser.add_argument(
        "--filter_score_threhold",
        default=0.5,
        type=float,
    ...
    parser.add_argument(
        "--log_to_file", action="store_true", help="Whether to log output"
    )
...
class Trainer:
    def __init__(self, args, rank=0, world_size=1, device_id=None, logger=None):
        ...
        self.sample_model = ProposalTargetLayer(
            args.n_classes,
        ...
        self.RCNN_model = RCNN(
            args.feature_input_dim,
            block,
            args.n_classes,
            args.output_size,
            is_add_layer=args.is_add_layer,
            is_rotated_bbox=args.rotated_bbox,
        ...
        if args.pretrained and args.fine_tune:
            assert os.path.exists(
                args.pretrained
            ), "The pretrained model does not exist."
    ...
    def init_datasets(self):
        ...
        if self.args.mode == "eval":
            self.test_set = RPNClassificationDataset(
            ...
            self.val_set = RPNClassificationDataset(
                self.args.features_path,
                self.args.boxes_path,
                self.args.rois_path,
    ...
    def build_backbone(self):
        if self.args.backbone_type == "resnet":
            ...
            self.backbone = VGG_FPN(
                "AF", self.args.backbone_input_dim, True, self.args.resolution
            )
        elif self.args.backbone_type == "vgg_EF":
            self.backbone = VGG_FPN(
                "EF", self.args.backbone_input_dim, True, self.args.resolution
            )
        ...
        torch.save(
            {
                "epoch": epoch,
                "backbone_state_dict": (
                    self.backbone.state_dict() if self.args.fine_tune else None
                ),
    ...
    def train_loop(self):
        self.train_set = RPNClassificationDataset(
            self.args.features_path,
            self.args.boxes_path,
            self.args.rois_path,
        ...
        for epoch in range(start_epoch, start_epoch + self.args.num_epochs):
            ...
            if self.rank != 0:
                continue
            if (
                epoch % self.args.eval_interval == 0
                or epoch == self.args.num_epochs - 1
            ):
                    ...
                    for metric_name in list(metric.keys()):
                        if metric[metric_name] > self.best_metric[metric_name]:
                            ...
                            self.save_checkpoint(
                                ...
                                os.path.join(
                                    self.args.save_path,
                                    f"model_best_ap{metric_name}.pt",
                                ),
                            )
    ...
    def train_epoch(self, epoch):
        for i, batch in enumerate(self.train_loader):
            ...
            proposals, scores, losses = self.model(
                rois, boxes, gt_labels, level_features
            )
    ...
    def output_proposals(self, scenes, proposals, scores, gt_boxes, threshold=0.7):
        ...
        for scene, proposal, score, gt in zip(scenes, proposals, scores, gt_boxes):
            ...
            np.savez(
                os.path.join(output_path, f"{threshold_name}", f"{scene}.npz"),
                proposal=proposal,
                score=score,
            )
    ...
    def filter_proposals(
        self, proposals, objectness, gt_labels, mesh_sizes, score_threhold=0.8
    ):
        final_boxes = []
        final_scores = []
        final_gt_labels = []
        bbox_size = 7 if self.args.rotated_bbox else 6
        for boxes, scores, gt_labels, mesh_shape in zip(
            proposals, objectness, gt_labels, mesh_sizes
        ):
    ...
    def eval(self, dataset):
        ...
        for batch in tqdm(dataloader):
            ...
            for b in range(len(rois_split)):
                ...
                for s in range(len(c_split) - 1):
                    ...
                    (
                        ...
                        [c_rois[c_split[s] : c_split[s + 1]]],
                        boxes,
                        gt_labels,
                        level_features,
                        is_sample=False,
                        is_reg=not self.args.obj_only,
                    )
            ...
            mesh_sizes = [
                level_features[iter][0].shape[1:] for iter in range(len(level_features))
            ]
            mesh_sizes = [
                [iter_iter * self.args.spatial_scale[0] for iter_iter in iter]
                for iter in mesh_sizes
            ]

            proposals, scores, gt_labels = self.filter_proposals(
                proposals[0],
                scores,
                proposals[1],
                mesh_sizes,
...
def main_worker(proc, nprocs, args, gpu_ids):
    ...
    dist.init_process_group(
        backend="nccl", init_method=args.init_method, world_size=nprocs, rank=proc
    )

...
def evaluate_box_proposals_recall(
    proposals_list, proposal_scores_list, gt_boxes_list, thresholds=None, limit=None
):
    """
    ...
    for proposals, scores, gt_boxes in zip(
        proposals_list, proposal_scores_list, gt_boxes_list
    ):
...
def evaluate_box_proposals_precision(
    proposals_list,
...
def evaluate_classificaiton(scores_list, gt_label_list, threshold):
    ...
    for cur_score, cur_gt_label in zip(scores_list, gt_label_list):
        ...
        cur_precision_100 = cur_gt_label[sort_index].sum() / cur_gt_label[
            sort_index
        ].size(0)
    ...
    return {
        ...
        "precision_100": (
            sum(precision_100) / len(precision_100) if len(precision_100) > 0 else 0
        ),
    }


def evaluate_box_proposals_average_precision(
    proposals_list, proposal_scores_list, gt_boxes_list, iou_thresh=0.25, top_k=None
):
    """
    ...
    for i, t in enumerate(conf_thresh):
        ...
        precisions[i] = (
            (box_overlaps[box_scores >= t] >= iou_thresh).float().sum() / num_dets[i]
            if num_dets[i] > 0
            else 0
        )
        recalls[i] = (
            (box_overlaps[box_scores >= t] >= iou_thresh).float().sum() / num_gt
            if num_gt > 0
            else 0
        )
...
def evaluate_box_proposals_ap(
    ...
    for i, (proposals, scores, gt_boxes) in enumerate(
        zip(proposals_list, proposal_scores_list, gt_boxes_list)
    ):
        if top_k is not None and len(proposals) > top_k:
    ...
    gt_used = [
        torch.zeros(len(gt_boxes), dtype=torch.bool) for gt_boxes in gt_boxes_list
    ]

...
cfg.trainer.opt.weight_decay = 0.0
cfg.trainer.opt.ema_decay = 0.9999
cfg.trainer.opt.grad_clip = -1.0
...
cfg.register_deprecated_key(
    "cond_encoder_prior"
)  # 'models.shapelatent_modules.VoxelGridEncoder'
...
cfg.register_deprecated_key(
    "lns.wandb_checkpoint"
)  # 'nvidia-toronto/generative_chairs/3m3gc6sz/checkpoint-171.pth'
...
cfg.register_deprecated_key(
    "shapelatent.refiner_bp"
)  # 1  # bp gradient to the local-decoder or not
cfg.register_deprecated_key(
    "shapelatent.loss_weight_refiner"
)  # 1.0  # weighted loss for the refiner
cfg.register_deprecated_key(
    "shapelatent.refiner_type"
)  # 'models.pvcnn2.PVCNN2BaseAPI'  # mode for the refiner
...
# reuse the encoder to get local latent
cfg.register_deprecated_key(
    "shapelatent.query_center_emd_from_enc"
)  # 0  # reuse the encoder for center emd
cfg.register_deprecated_key(
    "shapelatent.prog_dec_gf"
)  # 8  # grow_factor in VaniDecoderProg
cfg.register_deprecated_key(
    "shapelatent.prog_dec_gf_list"
)  # [0, 0]  # grow_factor in VaniDecoderProg
cfg.register_deprecated_key(
    "shapelatent.prog_dec_ne"
)  # 2  # num_expand in VaniDecoderProg
# increase number hirach, used by hvaemul model
cfg.register_deprecated_key(
    "shapelatent.num_neighbors_per_level"
)  # [64]  # number of neighbors for each level
cfg.register_deprecated_key(
    "shapelatent.num_level"
)  # 1  # number of hierarchi latent space (local)
cfg.register_deprecated_key(
    "shapelatent.x0_target_fps"
)  # 0  # let the target of global output as the
...
cfg.register_deprecated_key(
    "data.has_normal"
)  # 0  # for datasets/pointflow_rgb.py only
...
cfg.register_deprecated_key(
    "data.sample_curvature"
)  # 0  # only for datasets/shape_curvature

...
def plot_points(output, output_name=None):
    ...
    if output_name is None:
        ...
        output_name = os.path.join(
            output_dir, datetime.now().strftime("%y%m%d_%H%M%S.png")
        )
...
def visualize_point_clouds_3d_list(pcl_lst, title_lst, vis_order, vis_2D, bound, S):
    ...
    for i in range(len(pcl_lst)):
        img = visualize_point_clouds_3d(
            [pcl_lst[i]],
            [title_lst[i]] if title_lst is not None else None,
            vis_order,
            vis_2D,
            bound,
            S,
...
def visualize_point_clouds_3d(
    pcl_lst, title_lst=None, vis_order=[2, 0, 1], vis_2D=1, bound=1.5, S=3, rgba=0
):
    """
    ...
    Returns:
        image with $L$ column
    """
    assert type(pcl_lst) == list and torch.is_tensor(
        pcl_lst[0]
    ), f"expect list of tensor, get {type(pcl_lst)} and {type(pcl_lst[0])}"
    if len(pcl_lst) > 1:
        return visualize_point_clouds_3d_list(
            pcl_lst, title_lst, vis_order, vis_2D, bound, S
        )

...
def normalize_point_clouds(pcs, mode="shape_bbox"):  # can be a property func
    ...
    assert (
        type(pcs) == list or len(pcs.shape) == 3
    ), f"expect pcs to be list, get: {type(pcs)} or 3d tensor; "
    output_list = []
    for i in range(len(pcs)):  # , desc='Normalize'):
        pc = pcs[i]
        pc = pc.detach().clone()
        assert mode == "shape_bbox"
        assert len(pc.shape) == 2 and pc.shape[-1] in [
            3,
            4,
            6,
            9,
        ], f"expect get (N,3 or 6), get {pc.shape}"

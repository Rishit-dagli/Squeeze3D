...
def CHECKDIM(tensor, dim, val):
    if type(tensor) == list:
        ...
        assert len(tensor.shape) >= dim, "expect {} to have {} dim shape {}".format(
        ...
        if type(val) is list:
            ...
            assert (
                tensor.shape[dim] == val
            ), "expect tensor with shape: {} having dim {} as {}".format(
                tensor.shape, dim, val
            )
...
def CHECKND(tensor, N):
    assert len(tensor.shape) == N, "get tensor shape:{} DIM={}, expect:{}".format(
        tensor.shape, len(tensor.shape), N
    )
    return tensor.shape
...
def CHECK_N3or6(input):
    ...
    CHECK2D(input)
    assert (
        input.shape[1] == 3 or input.shape[1] == 6
    ), f"expect shape N,3 or N,6; get {input.shape}"
    return input.shape
...
def CHECK_N3or6or9(input):
    ...
    CHECK2D(input)
    assert (
        input.shape[1] == 3 or input.shape[1] == 6 or input.shape[1] == 9
    ), f"expect shape N,3 or N,6; get {input.shape}"
    return input.shape

...
from utils.utils import (
    trace_df_dx_hutchinson,
    sample_gaussian_like,
    sample_rademacher_like,
    get_mixed_prediction,
)
...
class DiffusionBase(ABC):
    ...
    def cross_entropy_const(self, ode_eps):
        ...
        return 0.5 * (
            1.0
            + torch.log(2.0 * np.pi * self.var(t=torch.tensor(ode_eps, device="cuda")))
        )
            ...
            with torch.set_grad_enabled(False):
                with autocast(enabled=enable_autocast):
                    ...
                    params = get_mixed_prediction(
                        dae.mixed_prediction,
                        pred_params,
                        dae.mixing_logit,
                        mixing_component,
                    )
                    dx_dt = self.f(t=t) * x + 0.5 * self.g2(t=t) * params / torch.sqrt(
                        variance
                    )
    ...
    def sample_model_ode(
        ...
        def ode_func(t, x):
            ...
            with autocast(enabled=enable_autocast):
                ...
                input_mixing_logit = (
                    mixing_logit if mixing_logit is not None else dae.mixing_logit
                )
                params = get_mixed_prediction(
                    dae.mixed_prediction,
                    pred_params,
    ...
    def iw_quantities(self, size, time_eps, iw_sample_mode, iw_subvp_like_vp_sde):
        if self.sde_type in ["geometric_sde", "vpsde", "power_vpsde"]:
            ...
            return self._iw_quantities_subvpsdelike(
                size, time_eps, iw_sample_mode, iw_subvp_like_vp_sde
            )
    ...
    def _iw_quantities_vpsdelike(self, size, time_eps, iw_sample_mode):
        ...
        if iw_sample_mode == "ll_uniform":
            ...
            obj_weight_t_p = obj_weight_t_q = (
                0.5 * (log_sigma2_1 - log_sigma2_eps) / (1.0 - var_t)
            )
            ...
            assert self.sde_type == "vpsde", (
                "Importance sampling for fully unweighted objective is currently only "
                "implemented for the regular VPSDE."
            )
            t = (
                torch.sqrt(1.0 / self.delta_beta_half)
                * torch.erfinv(rho * self.const_norm_2 + self.const_erf)
                - self.beta_frac
            )
            var_t, m_t, g2_t = self.var(t), self.e2int_f(t), self.g2(t)
...
class DiffusionVPSDE(DiffusionBase):
    ...
    def __init__(self, args):
        ...
        logger.info(
            "VPSDE: beta_start={}, beta_end={}, sigma2_0={}",
            self.beta_start,
            self.beta_end,
            self.sigma2_0,
        )
        ...
        self.const_aq = (
            (1.0 - self.sigma2_0)
            * torch.exp(0.5 * self.beta_frac)
            * torch.sqrt(0.25 * np.pi / self.delta_beta_half)
        )
        self.const_erf = torch.erf(
            torch.sqrt(self.delta_beta_half) * (self.time_eps + self.beta_frac)
        )
        self.const_norm = self.const_aq * (
            torch.erf(torch.sqrt(self.delta_beta_half) * (1.0 + self.beta_frac))
            - self.const_erf
        )
        self.const_norm_2 = (
            torch.erf(torch.sqrt(self.delta_beta_half) * (1.0 + self.beta_frac))
    ...
    def e2int_f(self, t):
        return torch.exp(
            -0.5 * self.beta_start * t
            - 0.25 * (self.beta_end - self.beta_start) * t * t
        )

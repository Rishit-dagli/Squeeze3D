...
import sys
import math
from utils.evaluation_metrics_fast import (
    compute_all_metrics,
...
def compute_NLL_metric(
    ...
    subtitles = [
        [
            "ori",
            "gen-%s=%.1fx1e-2;%s=%.1fx1e-2"
            % (k1, metrics[k1][j] * 1e2, k2, metrics[k2][j] * 1e2),
        ]
        for j in range(10)
    ]
    ...
    for k in metrics.keys():
        ...
        subtitles = [
            ["ori", "gen-%s=%.2fx1e-2" % (k, worse_score[j] * 1e2)]
            for j in range(len(worse_score))
        ]
        pair_vis(
            gen_pcs[worse_ten], ref_pcs[worse_ten], titles, subtitles, writer, step=step
        )
...
def get_ref_num(cats, luo_split=False):
    ...
    if luo_split:
        num_test = {"airplane": 607, "chair": 989, "car": 528}
...
def compute_score(
    ...
    **print_kwargs,
    ...
    Args:
        ...
        print_kwargs (dict): entries: dataset, hash, step, epoch;
    """
    logger.info("[compute sample metric] sample: {} and ref: {}", output_name, ref_name)
    ref = torch.load(ref_name)
    ref_pcs = ref["ref"][:, :, :3]
    m_pcs, s_pcs = ref["mean"], ref["std"]
    gen_pcs = torch.load(output_name)
    if gen_pcs.shape[1] > ref_pcs.shape[1]:
        xperm = np.random.permutation(np.arange(gen_pcs.shape[1]))[: ref_pcs.shape[1]]
        gen_pcs = gen_pcs[:, xperm]
    if type(gen_pcs) is dict:
        logger.info(
            "WARNING: the gen_pcs is a dict, with key "
    ...
    logger.info(
        "[data shape] ref_pcs: {}, gen_pcs: {}, mean={}, std={}; norm_box={}",
        ref_pcs.shape,
        gen_pcs.shape,
        m_pcs.shape,
        s_pcs.shape,
    ...
    if VIS:
        ...
        for i in range(20):
            NORM_VIS = 0
            if NORM_VIS:
                norm_ref, norm_gen = normalize_point_clouds([ref_pcs[i], gen_pcs[i]])
            else:
                norm_ref = ref_pcs[i]
                norm_gen = gen_pcs[i]
            ref_img = visualize_point_clouds_3d(
                [norm_ref], [f"ref-{i}"], bound=1.0
            )  # 0.8)
            gen_img = visualize_point_clouds_3d(
                [norm_gen], [f"gen-{i}"], bound=1.0
            )  # 0.8)
    ...
    results = compute_all_metrics(
        gen_pcs.to(device).float(),
        ref_pcs.to(device).float(),
        batch_size_test,
    ...
    if not skip_write:
        ...
        msg = write_results(
            os.path.join("./results/", "eval_out.csv"), results, **print_kwargs
        )
    if metric2 is None:

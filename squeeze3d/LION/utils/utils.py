...
class PixelNormal(object):
    ...
    def log_prob(self, samples):
        ...
        log_probs = (
            -0.5
            * torch.square(self.means - samples)
            * torch.exp(-2.0 * self.log_scales)
            - self.log_scales
            - 0.9189385332
        )  # -0.5*log(2*pi)
        return log_probs
    ...
    def sample(self, t=1.0):
        z, rho = sample_normal_jit(
            self.means, torch.exp(self.log_scales) * t
        )  # B, 3, H, W
        return z
    ...
    def log_prob_discrete(self, samples):
        ...
        log_one_minus_cdf_min = torch.log(torch.clamp(1.0 - cdf_min, min=1e-12))
...
def get_opt(params, cfgopt, use_ema, other_cfg=None):
    if cfgopt.type == "adam":
        ...
        logger.info(
            "[Optimizer] Adamax, lr={}, weight_decay={}, eps={}",
            cfgopt.lr,
            cfgopt.weight_decay,
            1e-4,
        )
        optimizer = Adamax(
            params, float(cfgopt.lr), weight_decay=args.weight_decay, eps=1e-4
        )
    ...
    def add_image(self, *args, **kwargs):
        ...
        if self.exp is not None:
            ...
            if isinstance(img, Image.Image):
                ...
                if img.max() < 100:  # [0-1]
                    ndarr = (
                        img.mul(255).add_(0.5).clamp_(0, 255).to("cpu")
                    )  # .squeeze()
...
def common_init(rank, seed, save_dir, comet_key=""):
    ...
    if rank == 0:
        if os.path.exists(".comet_api"):
            ...
            exp = Experiment(
                display_summary_level=0, disabled=USE_COMET == 0, **comet_args
            )
        ...
        if os.path.exists(".wandb_api"):
            ...
            wb_dir = (
                "../exp/wandb/"
                if not os.path.exists("/workspace/result")
                else "/workspace/result/wandb/"
            )
...
def kl_per_group_vada(all_log_q, all_neg_log_p):
    ...
    for log_q, neg_log_p in zip(all_log_q, all_neg_log_p):
        ...
        kl_all_list.append(
            torch.sum(neg_log_p + log_q, dim=[1, 2, 3])
        )  # sum over D,H,W
...
def kl_coeff(step, total_step, constant_step, min_kl_coeff, max_kl_coeff):
    ...
    return max(
        min(
            min_kl_coeff
            + (max_kl_coeff - min_kl_coeff) * (step - constant_step) / total_step,
            max_kl_coeff,
        ),
        min_kl_coeff,
    )
    ...
    if is_distributed:
        ...
        for param in params:
            if param.requires_grad and param.grad is not None:
                param.grad.data = grad_data[base : base + grad_size[i]].view(
                    grad_shapes[i]
                )
                base += grad_size[i]
                i += 1
...
def get_arch_cells(arch_type, use_se):
    if arch_type == "res_mbconv":
        ...
        arch_cells["normal_enc"] = {
            "conv_branch": ["res_bnswish", "res_bnswish"],
            "se": use_se,
        }
        arch_cells["down_enc"] = {
            "conv_branch": ["res_bnswish", "res_bnswish"],
...
def get_arch_cells_denoising(arch_type, use_se, apply_sqrt2):
    if arch_type == "res_mbconv":
        ...
        arch_cells["normal_enc_diff"] = {
            "conv_branch": ["mconv_e6k5g0_gn"],
            "se": use_se,
        }
        arch_cells["down_enc_diff"] = {"conv_branch": ["mconv_e6k5g0_gn"], "se": use_se}
        arch_cells["normal_dec_diff"] = {
            "conv_branch": ["mconv_e6k5g0_gn"],
            "se": use_se,
        }
        arch_cells["up_dec_diff"] = {"conv_branch": ["mconv_e6k5g0_gn"], "se": use_se}
    elif arch_type == "res_ho":
        arch_cells = dict()
        arch_cells["normal_enc_diff"] = {
            "conv_branch": ["res_gnswish_x2"],
            "se": use_se,
        }
        arch_cells["down_enc_diff"] = {"conv_branch": ["res_gnswish_x2"], "se": use_se}
        arch_cells["normal_dec_diff"] = {
            "conv_branch": ["res_gnswish_x2"],
            "se": use_se,
        }
        arch_cells["up_dec_diff"] = {"conv_branch": ["res_gnswish_x2"], "se": use_se}
    elif arch_type == "res_ho_p1":
        arch_cells = dict()
        arch_cells["normal_enc_diff"] = {
            "conv_branch": ["res_gnswish_x2_p1"],
            "se": use_se,
        }
        ...
        arch_cells["up_dec_diff"] = {"conv_branch": ["res_gnswish_x2_p1"], "se": use_se}
...
def calc_jacobian_regularization(
    pred_params, eps_t, dae, var_t, m_t, f_t, g2_t, var_N_t, args
):
    """
    ...
    for _ in range(args.jac_reg_samples):
        ...
        jvp = torch.autograd.grad(pred_params, eps_t, noise, create_graph=True)[0]
        ...
        if args.sde_type in ["geometric_sde", "vpsde", "power_vpsde"]:
            ...
            if not args.jac_kin_reg_drop_weights:
                jvp_ode_func = f_t * (1.0 - m_t**4) / torch.sqrt(var_t) * jvp_ode_func
        elif args.sde_type in ["vesde"]:
            jvp_ode_func = (1.0 - alpha) * noise * torch.sqrt(
                var_t
            ) / var_N_t + alpha * jvp
            if not args.jac_kin_reg_drop_weights:
        ...
        jvp_ode_func_norms.append(
            jvp_ode_func.view(eps_t.size(0), -1).pow(2).sum(dim=1, keepdim=True)
        )
...
def calc_kinetic_regularization(
    pred_params, eps_t, dae, var_t, m_t, f_t, g2_t, var_N_t, args
):
    """
    ...
    if args.sde_type in ["geometric_sde", "vpsde", "power_vpsde"]:
        ...
        ode_func = (1.0 - alpha) * eps_t * torch.sqrt(
            var_t
    ...
    kin_reg_loss = torch.mean(ode_func.view(eps_t.size(0), -1).pow(2).sum(dim=1))
    return kin_reg_loss


def different_p_q_objectives(iw_sample_p, iw_sample_q):
    assert iw_sample_p in [
        "ll_uniform",
        "drop_all_uniform",
        "ll_iw",
        "drop_all_iw",
...
def get_mixed_prediction(mixed_prediction, param, mixing_logit, mixing_component=None):
    if mixed_prediction:
        assert (
            mixing_component is not None
        ), "Provide mixing component when mixed_prediction is enabled."
...
def vae_regularization(args, vae_sn_calculator, loss_weight=None):
    ...
    regularization_q, vae_norm_loss, vae_bn_loss, vae_wdn_coeff = (
        ...
        0.0,
        0.0,
        args.weight_decay_norm_vae if loss_weight is None else loss_weight,
    )
...
def dae_regularization(
    ...
    if args.jac_reg_coeff > 0.0 and step % args.jac_reg_freq == 0:
        ...
        var_N_t = (
            diffusion.var_N(t).view(-1, 1, 1, 1) if args.sde_type == "vesde" else None
        )
        """
        ...
        """
        jac_reg_loss = calc_jacobian_regularization(
            pred_params_p, eps_t_p, dae, var_t_p, m_t_p, f_t, g2_t_p, var_N_t, args
        )
    ...
    if args.kin_reg_coeff > 0.0:
        ...
        """
        kin_reg_loss = calc_kinetic_regularization(
            pred_params_p, eps_t_p, dae, var_t_p, m_t_p, f_t, g2_t_p, var_N_t, args
        )
...
def update_lr(
    ...
    if global_step < warmup_iters:
        ...
        if args.learning_rate_mlogit > 0 and len(dae_optimizer.param_groups) > 1:
            lr_mlogit = args.learning_rate_mlogit * float(global_step) / warmup_iters
        ...
        lr = (
            lr
            if args.learning_rate_dae_local <= 0
            else args.learning_rate_dae_local * float(global_step) / warmup_iters
        )

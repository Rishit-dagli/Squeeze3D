...
def pts2png(
    ...
    ball_size=0.025,
    do_standardize=0,
    same_computed_loc_color=0,
    material_id=0,
    precomputed_color=None,
    ...
    for pcli in range(0, input_pts.shape[0]):
        xmlFile = (
            "/tmp/tmp_%s.xml" % random_str
            if output_xml_file is None
            else output_xml_file
        )
        # ("%s/xml/%s.xml" % (folder, filename))
        exrFile = (
            "/tmp/tmp_%s.exr" % random_str
        )  ##("%s/exr/%s.exr" % (folder, filename))
        png = file_name[pcli]
        ...
        for i in range(pcl.shape[0]):
            ...
            xml_segments.append(
                xml_ball_segment[material_id].format(
                    ball_size, pcl[i, 0], pcl[i, 1], pcl[i, 2], *color
                )
            )

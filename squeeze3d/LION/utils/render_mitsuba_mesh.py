...
def main(
    ...
    xml_head = xml_head_segment.format(
        lookat_1, lookat_2, lookat_3, sample_count, out_width, out_height
    )
    xml_segments = [xml_head]
    color = [c / 255.0 for c in colorm]
    if material_id in [2, 3]:
        xml_segments.append(xml_shape_segment[material_id].format(pathToMesh))
    else:
        xml_segments.append(xml_shape_segment[material_id].format(pathToMesh, *color))

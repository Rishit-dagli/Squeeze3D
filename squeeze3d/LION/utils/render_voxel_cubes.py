...
def reformat_ply(
    ...
    open3d.io.write_triangle_mesh(
        output, mesh_r, write_ascii=ascii, write_vertex_normals=False
    )
    if fixed_trimesh:
        ...
        mesh.export(output)
    logger.info(
        f"load {input}, write as {output}; ascii={ascii}, write_normal: {write_normal}"
    )
    return output
...
def get_vpts(cubes):
    ...
    v, f = kaolin.ops.conversions.voxelgrids_to_cubic_meshes(
        cubes
    )  ##voxelgrids_to_trianglemeshes(voxel_volume)
...
def convert_cube_2_mesh(
    ...
    pcl = center_list
    mins = np.amin(pcl, axis=0).reshape(
        1, pcl.shape[-1]
    )  ##np.amin(pcl, axis=1)[:, None, :], axis=0)[None, None, :]
    maxs = np.amax(pcl, axis=0).reshape(
        1, pcl.shape[-1]
    )  ##np.amin(pcl, axis=1)[:, None, :], axis=0)[None, None, :]
    ...
    center_list = (
        center_list - center
    ) / scale  ## (center_list.max() - center_list.min())
    ...
    if os.path.exists(output_name) and not overwrite:
        ...
        return
    out = render_cubes2png(
        pcl=center_list,
        filename=output_name,
        vs_size=0.9 * voxel_size / scale,
        colorm=colorm,
        **config,
    )
    print(" save as: ", out)
...
def write_small_cube():
    ...
    o3d.io.write_triangle_mesh(
        mesh_compare.replace(".ply", "w1.ply"),
        m,
        write_ascii=True,

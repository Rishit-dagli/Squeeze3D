...
class DiffusionDiscretized(object):
    ...
    def __init__(self, args, var_fun, cfg):  # alpha_bars_fun
        ...
        self.betas = make_beta_schedule(mode, beta_start, beta_end, num_steps).numpy()
        (
            self._betas_init,
            self._alphas,
            self._alpha_bars,
            self._betas_post_init,
            self.snr,
    ...
    def iw_quantities_t(self, B, timestep, *args):
        ...
        if self.use_p2_weight:
            p2_weight = torch.gather(
                1 / (self.p2_k + self.snr) ** self.p2_gamma, 0, timestep - 1
            ).view(B)
    ...
    def iw_quantities(self, B, *args):
        ...
        assert (
            timestep.max() <= self._diffusion_steps - 1
        ), f"get max at {timestep.max()}"
        timestep = timestep + 1  # [1,T]
        alpha_bars = torch.gather(self._alpha_bars, 0, timestep - 1)  # [0,T-1]
        weight_init = alpha_bars_sqrt = torch.sqrt(alpha_bars)
        weight_noise_power = 1.0 - alpha_bars
    ...
    def _generate_base_constants(self, diffusion_steps):
        ...
        betas_np = (
            self.betas
        )  # self._generate_betas_from_continuous_fun(diffusion_steps)
        ...
        betas_post_np = (
            betas_np[1:] * (1.0 - alpha_bars_np[:-1]) / (1.0 - alpha_bars_np[1:])
        )
    ...
    def run_denoising_diffusion(
        ...
        for t in reversed(range(0, self._diffusion_steps)):
            if t % 500 == 0:
                logger.info(
                    "t={}; shape={}, num_samples={}, sample shape: {}",
                    t,
                    shape,
                    num_samples,
                    x_noisy.shape,
            ...
            timestep = torch.ones(num_samples, dtype=torch.int64, device="cuda") * (
                t + 1
            )
            ...
            with autocast(enable_autocast):
                if cls_emb is not None and condition_input is not None:
                    condition_input = torch.cat([condition_input, cls_emb], dim=1)
        ...
        if is_image:
            x_image = x_image.clamp(min=-1.0, max=1.0)
            x_image = utils.unsymmetrize_image_data(x_image)
        model.train()
        return x_image, output_list

    def run_ddim_forward(
        self, dae, eps, ddim_step, ddim_skip_type, condition_input=None, clip_feat=None
    ):
        ...
        if skip_type == "uniform":
            ...
            seq = np.linspace(0, np.sqrt(self._diffusion_steps * 0.8), S) ** 2
    ...
    def run_ddim(
        ...
        for i, t in enumerate(user_defined_steps):
            ...
            x += c * epsilon_theta + sigma * torch.randn(x_noisy_size).to(x.device)
            x_noisy = x
            output_list.append(x_noisy)
        # if is_image:
        #     x_image = x_image.clamp(min=-1., max=1.)
    ...
    def get_q_posterior_mean(self, x_noisy, prediction, t):
        ...
        if t == 0:
            mean = (
                1.0
                / torch.sqrt(self._alpha_bars[0])
                * (x_noisy - torch.sqrt(1.0 - self._alpha_bars[0]) * prediction)
            )
        else:
            mean = (
                1.0
                / torch.sqrt(self._alphas[t])
                * (
                    x_noisy
                    - self._betas_init[t]
                    * prediction
    ...
    @torch.no_grad()
    def run_denoising_diffusion_from_t(
        self,
        model,
        num_samples,
        shape,

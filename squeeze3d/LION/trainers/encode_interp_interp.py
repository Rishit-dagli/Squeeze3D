...
class Trainer(BaseTrainer):
    ...
    def eval_nll(self, step, ntest=None, save_file=False):
        ...
        tag = (
            exp_helper.get_evalname(self.cfg)
            + "D%d" % self.cfg.voxel2pts.diffusion_steps[0]
        )
        ...
        output_dir_template = self.cfg.save_dir + "/enc%d_%s_%s/" % (
            num_selected,
            self.generate_mode_global,
            self.generate_mode_local,
        )
        vis_output_dir = self.cfg.save_dir + "/vis_enc%d_%s_%s/" % (
            num_selected,
            self.generate_mode_global,
            self.generate_mode_local,
        )
        ...
        for vid, val_batch in enumerate(data_loader):
            ...
            log_info = {
                "x_0": val_x,
                "vis/input_noise": inputs.view(B, N, -1),
                "x_t": val_x,
            }
            ...
            eps_T_global_interp = diffusion.compute_ode_nll(
                dae[0], eps_0, ode_eps, ode_solver_tol, condition_input=None
            )
            ...
            eps_T_local_interp = diffusion.compute_ode_nll(
                dae[1], eps, ode_eps, ode_solver_tol, condition_input=eps_global
            )
            ...
            gen_x = self.model.decoder(
                None, beta=None, context=eps_local, style=style
            )  # (B,ncenter,3)
            ## gen_x = val_x
            ...
            output = self.model.get_loss(
                val_x, it=step, is_eval_nll=1, noisy_input=inputs
            )
            # gen_x = gen_x.cpu() * s + m

...
class BaseTrainer(ABC):
    ...
    def epoch_end(self, epoch, writer=None, **kwargs):
        ...
        if self.scheduler is not None:
            ...
            if writer is not None:
                writer.add_scalar("train/opt_lr", self.scheduler.get_lr()[0], epoch)
        if writer is not None:
    ...
    def save(
        ...
        **kwargs,
        ...
        save_name = (
            "epoch_%s_iters_%s.pt" % (epoch, step) if save_name is None else save_name
        )
    ...
    def train_epochs(self):
        ...
        logger.info(
            "[rank=%d] Start epoch: %d End epoch: %d, batch-size=%d | "
        ...
        for epoch in range(self.start_epoch, cfg.trainer.epochs):
            ...
            if (
                args.global_rank == 0
                and cfg.trainer.type in ["trainers.voxel2pts", "trainers.voxel2pts_ada"]
                and epoch == 0
            ):
            ...
            for bidx, data in enumerate(train_loader):
                ...
                # -- train for one iter -- #
                logs_info = self.train_iter(data, step=step, **self.train_iter_kwargs)
                ...
                if (
                    step % int(cfg.viz.log_freq) == 0
                    and args.global_rank == 0
                    and not (
                        step == 0
                        and cfg.sde.ode_sample
                        and (
                            cfg.trainer.type == "trainers.train_prior"
                            or cfg.trainer.type == "trainers.train_2prior"
                        )  # this case, skip sampling at first step
                    )
                ):
                    avg_loss = np.array(epoch_loss).mean()
                    epo_loss = []  # clean up epoch loss
                    self.log_loss({"epo_loss": avg_loss}, writer=writer, step=step)
                    visualize = (
                        int(cfg.viz.viz_freq) > 0
                        and (step) % int(cfg.viz.viz_freq) == 0
                    )
                    ...
                    if visualize:
                        self.model.eval()
                        self.vis_sample(writer, step=step, include_pred_x0=False)
                        self.model.train()
            ...
            if args.global_rank == 0:
                ...
                logger.info(
                    ...
                    % (
                        ...
                        writer.url,
                        epo_time,
                        epo_time * (cfg.trainer.epochs - epoch) / 60,
                        self.best_eval_epoch,
                        self.best_eval_score * 1e2,
                    )
                )
            ...
            ## -- save model -- ##
            if (
                (epoch + 1) % int(cfg.viz.save_freq) == 0
                and int(cfg.viz.save_freq) > 0
                and args.global_rank == 0
            ):
                self.save(epoch=epoch, step=step)
            if (
                (time.time() - tic0) / 60 > cfg.snapshot_min
            ) and args.global_rank == 0:  # save every 30 min
                file_name = self.save(save_name="snapshot_bak", epoch=epoch, step=step)
                if file_name is None:
                    file_name = os.path.join(
                        self.cfg.save_dir, "checkpoints", "snapshot_bak"
                    )
                os.rename(file_name, file_name.replace("snapshot_bak", "snapshot"))
                tic0 = time.time()

            ## -- run eval -- ##
            if (
                int(cfg.viz.val_freq) > 0
                and (epoch + 1) % int(cfg.viz.val_freq) == 0
                and args.global_rank == 0
            ):
                eval_score = self.eval_nll(step=step, save_file=False)
                if eval_score < self.best_eval_score or self.best_eval_score < 0:
                    self.save(
                        save_name="best_eval.pth",  # save_dir=snapshot_dir,
                        epoch=epoch,
                        step=step,
                    )
        ...
        if args.global_rank == 0:
            self.eval_nll(step=step)
        if (
            self.cfg.trainer.type == "trainers.train_prior"
        ):  # and args.global_rank == 0:
            self.model.eval()
    ...
    def vis_recont(self, output, writer, step, normalize_pts=False):
        ...
        x_0_pred, x_0, x_t = (
            output.get("x_0_pred", None),
            output.get("x_0", None),
            output.get("x_t", None),
        )
        if x_0_pred is None or x_0 is None or x_t is None:
            logger.info(
                "x_0_pred: None? {}; x_0: None? {}, x_t: None? {}",
                x_0_pred is None,
                x_0 is None,
                x_t is None,
            )
    ...
    def eval_sample(self, step=0):
        ...
        num_ref = get_ref_num(cates) if self.cfg.num_ref == 0 else self.cfg.num_ref

        # option for post-processing
        if (
            self.cfg.data.recenter_per_shape
            or self.cfg.data.normalize_shape_box
            or self.cfg.data.normalize_range
        ):
            norm_box = True
        else:
            norm_box = False
        logger.info(
            "norm_box: {}, recenter : {}, shapebox: {}",
            norm_box,
            self.cfg.data.recenter_per_shape,
        ...
        output_name = os.path.join(self.cfg.save_dir, f"samples_{step}{tag}.pt")
        logger.info(
            "batch_size_test={}, test_size={}, saved output: {} ",
            batch_size_test,
            test_size,
            output_name,
        )
        ...
        if True:
            ...
            index_start = 0
            logger.info(
                "Rank={}, num_gen_iter: {}; num_ref={}, batch_size_test={}",
            ...
            for i in range(0, num_gen_iter):
                ...
                logger.info("#%d/%d; BS=%d" % (i, num_gen_iter, batch_size_test))
                # ---- draw samples ---- #
                self.index_start = index_start
                x = (
                    self.sample(
                        num_shapes=batch_size_test,
                ...
                assert (
                    x.shape[-1] == input_dim
                ), f"expect x: B,N,{input_dim}; get {x.shape}"
        ...
        if self.args.distributed:
            ...
            logger.info(
                "before gather: {}, rank={}", gen_pcs.shape, self.args.global_rank
            )
            gen_pcs_list = [
                torch.zeros_like(gen_pcs) for _ in range(self.args.global_size)
            ]
            dist.all_gather(gen_pcs_list, gen_pcs)
            gen_pcs = torch.cat(gen_pcs_list, dim=0).cpu()
            logger.info(
                "after gather: {}, rank={}", gen_pcs.shape, self.args.global_rank
            )
        ...
        if writer is not None:
            ...
            for i in range(1):
                ...
                img = visualize_point_clouds_3d(
                    norm_ref, [f"gen-{k}" for k in range(len(norm_ref))]
                )
                img_list.append(torch.as_tensor(img) / 255.0)
            grid = torchvision.utils.make_grid(img_list)
            logger.info(
                "ndarr: {}, range: {} img list: {} ",
                grid.shape,
                grid.max(),
                img_list[0].shape,
                img_list[0].max(),
        ...
        results = compute_score(
            ...
            ref_name=ref,
            writer=writer,
            batch_size_test=min(5, self.cfg.data.batch_size_test),
            norm_box=norm_box,
            **print_kwargs,
        )
        ...
        msg += "\n[Test] MinMatDis | CD %.6f | EMD %.6f" % (
            results["lgan_mmd-CD"],
            results["lgan_mmd-EMD"],
        )
        msg += "\n[Test] Coverage  | CD %.6f | EMD %.6f" % (
            results["lgan_cov-CD"],
            results["lgan_cov-EMD"],
        )
        msg += "\n[Test] 1NN-Accur | CD %.6f | EMD %.6f" % (
            results["1-NN-CD-acc"],
            results["1-NN-EMD-acc"],
        )
    ...
    def vis_sample(
        self, writer, num_vis=None, step=0, include_pred_x0=True, save_file=None
    ):
        if num_vis is None:
        ...
        all_imgs_torchvis_norm = (
            []
        )  # no preconcat in the image, left to the torchvision
        for idx in range(num_vis):
            ...
            img.append(
                visualize_point_clouds_3d(norm_pcs, title_list, self.cfg.viz.viz_order)
            )
            ...
            if include_pred_x0:
                ...
                img.append(
                    visualize_point_clouds_3d(
                        traj_x0[idx][0:1], title_list, self.cfg.viz.viz_order
                    )
                )
    ...
    def prepare_vis_data(self):
        ...
        self.prior_cond = (
            torch.cat(prior_cond, dim=0)[:num_val_samples].to(device)
            if len(prior_cond)
            else None
        )
        self.val_input = (
            torch.cat(val_input, dim=0)[:num_val_samples].to(device)
            if len(val_input)
            else None
        )
        c = 0
        ...
        tr_cls = []
        logger.info(
            "[prepare_vis_data] len of train_loader: {}", len(self.train_loader)
        )
        assert (
            len(self.train_loader) > 0
        ), f"get zero length train_loader, it could be the batch_size > the number of training sample, and the train drop_last is turn off"
        ...
        self.w_prior = torch.randn(
            [num_val_samples, self.cfg.shapelatent.latent_dim]
        ).to(device)
        if self.clip_feat_list is not None:
            ...
            logger.info("[VIS data] clip_feat_test: {}", self.clip_feat_test.shape)
    ...
    def eval_nll(self, step, ntest=None, save_file=False):
        ...
        for vid, val_batch in enumerate(data_loader):
            ...
            s = s.view(B, 1, -1)
            inputs = (
                val_batch["input_pts"].to(device) if "input_pts" in val_batch else None
            )  # the noisy points
            model_kwargs = {}
            ...
            if ntest is not None and shape_id_start >= int(ntest):
                logger.info(
                    "!! reach number of test={}; has test: {}", ntest, shape_id_start
                )
                break
            shape_id_start += B
        ...
        if save_file:
            logger.info(
                "reconstruct point clouds..., output shape: {}, save as {}",
                gen_pcs.shape,
                output_name,
            )
            torch.save(gen_pcs, output_name)
    ...
    def prepare_clip_model_data(self):
        ...
        self.clip_model, self.clip_preprocess = clip.load(
            cfg.clipforge.clip_model, device=self.device_str
        )
        self.test_img_path = []
        ...
        if len(self.test_img_path):
            self.test_img = [Image.open(t).convert("RGB") for t in self.test_img_path]
            self.test_img = [
                self.clip_preprocess(img).unsqueeze(0).to(self.device_str)
        ...
        if len(self.test_img):
            clip_feat.append(self.clip_model.encode_image(self.test_img).float())

...
def init_optimizer_train_2prior(cfg, vae, dae, cond_enc=None):
    ...
    if args.use_adamax:
        ...
        dae_optimizer = Adamax(
            param_dict_dae,
            args.learning_rate_dae,
            weight_decay=args.weight_decay,
            eps=1e-4,
        )
    elif args.use_adam:
        cfgopt = cfg.trainer.opt
        dae_optimizer = optim.Adam(
            param_dict_dae,
    ...
    if args.use_adamax:
        ...
        vae_optimizer = Adamax(
            vae.parameters(),
            args.learning_rate_vae,
            weight_decay=args.weight_decay,
            eps=1e-3,
        )
    elif args.use_adam:
    ...
    vae_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        vae_optimizer,
        float(args.epochs - args.warmup_epochs - 1),
        eta_min=args.learning_rate_min_vae,
    )
    ...
    if vis_latent_point and num_samples > 0:
        ...
        for i in range(num_samples):
            ...
            if prior_cond is not None:
                ...
                img2 = visualize_point_clouds_3d([points], ["cond_center"], **vis_args)
        ...
        prior_cond_list = (
            np.concatenate(prior_cond_list, axis=2)
            if len(prior_cond_list)
            else prior_cond_list
        )
        img = np.concatenate([img, img_eps], axis=1)
        img = (
            np.concatenate([img, prior_cond_list], axis=1)
            if len(prior_cond_list)
            else img
        )
        writer.add_image("sample", torch.as_tensor(img), it)
...
def generate_samples_vada_2prior(
    ...
    if ode_sample == 1:
        ...
        if cfg.data.cond_on_voxel:
            ...
            voxel_grid_enc_out = dae[2](
                prior_cond.to(device)
            )  # embed the condition_input

        ...
        for vid, val_batch in enumerate(data_loader):
            ...
            for time_start in self.cfg.voxel2pts.diffusion_steps:
                ...
                if diffusion_in_latent and time_start > 0:
                    ...
                    t, var_t_q, m_t_q, _, _, _ = diffusion.iw_quantities_t(
                        B, (torch.zeros(B) + time_start).cuda().long()
                    )
                    eps_t_q = diffusion.sample_q(eps, noise_q, var_t_q, m_t_q)
                    eps, _ = diffusion.run_denoising_diffusion_from_t(
                        self.dae[dae_index],
                        B,
                        shape,
                        time_start,
            ...
            for time_start in self.cfg.voxel2pts.diffusion_steps:
                ...
                if diffusion_in_latent and time_start > 0:
                    ...
                    eps_t_q = diffusion.sample_q(eps, noise_q, var_t_q, m_t_q)
                    log_info["vis/eps_t%d" % time_start] = eps_t_q.view(B, N, -1).cpu()[
                        :, :, :3
                    ]
                    eps, _ = diffusion.run_denoising_diffusion_from_t(
                        self.dae[dae_index],

...
class Trainer(BaseTrainer):
    def __init__(self, cfg, args):
        ...
        if len(self.cfg.sde.vae_checkpoint):
            ...
            self.model.load_state_dict(torch.load(self.cfg.sde.vae_checkpoint)["model"])
        ...
        self.optimizer, self.scheduler = utils.get_opt(
            self.model.parameters(), self.cfg.trainer.opt, cfg.ddpm.ema, self.cfg
        )
    ...
    def train_iter(self, data, *args, **kwargs):
        ...
        warmup_iters = (
            len(self.train_loader) * self.cfg.trainer.opt.vae_lr_warmup_epochs
        )
        ...
        with autocast(enabled=self.cfg.sde.autocast_train):
            res = self.model.get_loss(
                tr_pts, writer=self.writer, it=step, **model_kwargs
            )
            loss = res["loss"].mean()
        ...
        if not no_update:
            ...
            utils.average_gradients(self.model.parameters(), self.args.distributed)
            if self.cfg.trainer.opt.grad_clip > 0:
                torch.nn.utils.clip_grad_norm_(
                    self.model.parameters(), max_norm=self.cfg.trainer.opt.grad_clip
                )
        ...
        if self.writer is not None:
            for k, v in res.items():
                if "print/" in k and step is not None:
                    ...
                    self.writer.avg_meter(k.split("print/")[-1], v0, step=step)
                if "hist/" in k:
                    output[k] = v
    ...
    def sample(
        ...
        # ---- forward sampling ---- #
        gen_x = self.model.sample(num_samples=num_shapes, device_str=self.device_str)
        # gen_x: BNC

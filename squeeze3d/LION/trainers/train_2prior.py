...
@torch.no_grad()
def generate_samples_vada_2prior(
    shape,
    dae,
    diffusion,
    vae,
        ...
        for i in range(len(dae)):
            ...
            if cls_emb is not None:
                condition_input = torch.cat(
                    [condition_input, cls_emb.unsqueeze(-1).unsqueeze(-1)], dim=1
                )
            if i == 0:
    ...
    image = vae.sample(
        num_samples=num_samples, decomposed_eps=decomposed_eps, cls_emb=cls_emb
    )
...
class Trainer(PriorTrainer):
    ...
    def train_iter(self, data, *args, **kwargs):
        ...
        inputs = (
            data["input_pts"].to(device) if "input_pts" in data else None
        )  # the noisy points
        ...
        if tr_img is not None:
            ...
            clip_feat = (
                self.clip_model.encode_image(tr_img).view(B, nimg, -1).mean(1).float()
            )
        else:
            clip_feat = None
        ...
        if args.train_dae:
            ...
            with autocast(enabled=args.autocast_train):
                ...
                output["vis/eps"] = decomposed_eps[1].view(
                    -1, self.dae.num_points, self.dae.num_classes
                )[:, :, :3]
                p_loss_list = []
                for latent_id, eps in enumerate(decomposed_eps):
                    ...
                    if latent_id == 0:
                        ...
                        condition_input = (
                            decomposed_eps[0]
                            if not self.cfg.data.cond_on_cat
                            else torch.cat(
                                [
                                    decomposed_eps[0],
                                    output["cls_emb"].unsqueeze(-1).unsqueeze(-1),
                                ],
                                dim=1,
                            )
                        )
                        condition_input = self.model.global2style(condition_input)
                        pred_params_p = dae[latent_id](
                            eps_t_p,
                            t_p,
                            x0=eps,
                    ...
                    pred_eps_t0 = (
                        eps_t_p - torch.sqrt(var_t_p) * pred_params_p
                    ) / m_t_p

                    params = utils.get_mixed_prediction(
                        args.mixed_prediction,
                        pred_params_p,
                        dae[latent_id].mixing_logit,
                        mixing_component,
                    )
    ...
    def build_prior(self):
        ...
        DAE = nn.ModuleList(
            [
                import_model(self.cfg.latent_pts.style_prior)(
                    args, self.cfg.latent_pts.style_dim, self.cfg
                ),  # global latent prior
                import_model(self.cfg.sde.prior_model)(
                    args, num_input_channels, self.cfg
                ),  # latent point prior, conditional model
            ]
        )

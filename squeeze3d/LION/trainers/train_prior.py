...
def generate_samples_vada(
    ...
    if ode_sample == 1:
        ...
        ), "ODE-based sampling requires cont. diffusion!"
        assert (
            ode_eps is not None
        ), "ODE-based sampling requires integration cutoff ode_eps!"
        assert (
            ode_solver_tol is not None
        ), "ODE-based sampling requires ode solver tolerance!"
        start = timer()
        ...
        ), "Regular sampling requires disc. diffusion!"
        ...
        start = timer()
        if ddim_step > 0:
            eps, eps_list = diffusion.run_ddim(
                dae,
                num_samples,
                shape,
                temp,
            ...
            eps, eps_list = diffusion.run_denoising_diffusion(
                dae,
                num_samples,
                shape,
                temp,
...
def validate_inspect(
    ...
    vis_latent_point=False,
    ddim_step=0,
    epoch=0,
    fun_generate_samples_vada=None,
    clip_feat=None,
    cls_emb=None,
    cfg={},
):
    ...
    if need_sample:
        # gen_x: B,N,3
        gen_x, nstep, ode_time, sample_time, output_dict = fun_generate_samples_vada(
            latent_shape,
            dae,
            diffusion,
            model,
        ...
        logger.info(
            "cast={}, sample step={}, ode_time={}, sample_time={}",
            autocast_train,
    ...
    if vis_latent_point and num_samples > 0:
        ...
        eps = output_dict["sampled_eps"].view(
            num_samples, dae.num_points, dae.num_classes
        )[:, :, :3]
        for i in range(num_samples):
            points = gen_x[i]
            points = normalize_point_clouds([points])[0]
    ...
    output = (
        model.recont(inputs)
        if cls_emb is None
        else model.recont(inputs, cls_emb=cls_emb)
    )
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, cfg, args):
        ...
        if (
            len(self.cfg.sde.vae_checkpoint)
            and not args.pretrained
            and self.cfg.sde.vae_checkpoint != "none"
        ):
        ...
        utils.broadcast_params(self.model.parameters(), args.distributed)
        ...
        self.alpha_i = utils.kl_balancer_coeff(
            num_scales=2, groups_per_scale=[1, 1], fun="square"
        )
    ...
    def init_optimizer(self):
        ...
        self.dae_sn_calculator, self.vae_sn_calculator = (
            out_dict["dae_sn_calculator"],
            out_dict["vae_sn_calculator"],
        )
        self.vae_scheduler, self.vae_optimizer = (
            out_dict["vae_scheduler"],
            out_dict["vae_optimizer"],
        )
        self.dae_scheduler, self.dae_optimizer = (
            out_dict["dae_scheduler"],
            out_dict["dae_optimizer"],
        )
    ...
    def save(
        ...
        **kwargs,
        ...
        content = {
            ...
            "dae_optimizer": dae_optimizer.state_dict(),
            "dae_scheduler": dae_scheduler.state_dict(),
            "vae_state_dict": vae.state_dict(),
            "vae_optimizer": vae_optimizer.state_dict(),
            "vae_scheduler": vae_scheduler.state_dict(),
        }
    ...
    def train_iter(self, data, *args, **kwargs):
        ...
        dae.train()
        diffusion = (
            self.diffusion_cont if self.cfg.sde.ode_sample else self.diffusion_disc
        )
        ...
        utils.update_lr(args, global_step, warmup_iters, dae_optimizer, vae_optimizer)
        ...
        if args.train_vae:
            ...
            if args.grad_clip_max_norm > 0.0:  # apply gradient clipping
                grad_scalar.unscale_(vae_optimizer)
                torch.nn.utils.clip_grad_norm_(
                    vae.parameters(), max_norm=args.grad_clip_max_norm
                )
        ...
        if args.train_dae:
            ...
            with autocast(enabled=args.autocast_train):
                ...
                t_p, var_t_p, m_t_p, obj_weight_t_p, _, g2_t_p = (
                    diffusion.iw_quantities(
                        B, args.time_eps, args.iw_sample_p, args.iw_subvp_like_vp_sde
                    )
                )
                ...
                pred_eps_t0 = (eps_t_p - torch.sqrt(var_t_p) * pred_params_p) / m_t_p
                params = utils.get_mixed_prediction(
                    args.mixed_prediction,
                    pred_params_p,
                    dae.mixing_logit,
                    mixing_component,
                )
                if self.cfg.latent_pts.pvd_mse_loss:
                    ...
                    if args.regularize_mlogit:
                        reg_mlogit = (
                            (
                                torch.sum(torch.sigmoid(dae.mixing_logit))
                                - args.regularize_mlogit_margin
                            )
                            ** 2
                        ) * args.regularize_mlogit
                    else:
                        reg_mlogit = 0
                    p_loss = torch.mean(p_objective) + regularization_p + reg_mlogit
            ...
            if args.grad_clip_max_norm > 0.0:  # apply gradient clipping
                ...
                torch.nn.utils.clip_grad_norm_(
                    dae.parameters(), max_norm=args.grad_clip_max_norm
                )
            ...
            if writer is not None:
                writer.avg_meter(
                    "train/lr_dae",
                    dae_optimizer.state_dict()["param_groups"][0]["lr"],
                    global_step,
                )
                writer.avg_meter(
                    "train/lr_vae",
                    vae_optimizer.state_dict()["param_groups"][0]["lr"],
                    global_step,
                )
                if self.cfg.latent_pts.pvd_mse_loss:
                    writer.avg_meter("train/p_loss", p_loss.item(), global_step)
                    ...
                    if args.regularize_mlogit:
                        writer.avg_meter(
                            "train/m_logit",
                            reg_mlogit / args.regularize_mlogit,
                            global_step,
                        )
                    if args.mixed_prediction:
                        writer.avg_meter(
                            "train/m_logit_sum",
                            torch.sum(torch.sigmoid(dae.mixing_logit)).detach().cpu(),
                            global_step,
                        )
        ...
        output_dict = {
            ...
            "x_t": res["final_pred"]
            .detach()
            .view(batch_size, -1, res["x_0"].shape[-1]),
            "t": res.get("t", None),
        }
    ...
    def vis_sample(
        ...
        logger.info(
            "Latent shape for prior: {}; num_val_samples: {}",
            shape,
    ...
    def sample(
        ...
        assert (
            not self.cfg.clipforge.enable
        ), f"not suuport yet, not sure what the clip feat will be"
        cfg = self.cfg
        if cfg.ddpm.ema:
            self.swap_vae_param_if_need()
            self.dae_optimizer.swap_parameters_with_ema(store_params_in_ema=True)
        ...
        S = self.num_steps
        logger.info(
            "num_shapes={}, num_points={}, use_ddim={}, Nstep={}",
            num_shapes,
            num_points,
            use_ddim,
            S,
        ...
        gen_x, nstep, ode_time, sample_time, output_fsample = (
            self.fun_generate_samples_vada(
                latent_shape,
                self.dae,
                diffusion,
                self.model,
        ...
        if gen_x.shape[1] > self.sample_num_points:
            gen_x = (
                pvcnn_fn.furthest_point_sample(
                    gen_x.permute(0, 2, 1).contiguous(), self.sample_num_points
                )
                .permute(0, 2, 1)
                .contiguous()
            )  # [B,C,npoint]
    ...
    def build_prior(self):
        args = self.cfg.sde
        device = torch.device(self.device_str)
        arch_instance_dae = utils.get_arch_cells_denoising("res_ho_attn", True, False)
        ...
        if len(self.cfg.sde.dae_checkpoint):
            logger.info("Load dae checkpoint: {}", self.cfg.sde.dae_checkpoint)
            checkpoint = torch.load(self.cfg.sde.dae_checkpoint, map_location="cpu")
        ...
        logger.info("DAE: param size = %fM " % utils.count_parameters_in_M(self.dae))

...
class PVCNN2Unet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        logger.info(
            "[Build Unet] extra_feature_channels={}, input_dim={}",
...
class PointTransPVC(nn.Module):
    ...
    def forward(self, inputs):
        ...
        B, N, D = x.shape
        output = (
            self.layers(x.permute(0, 2, 1).contiguous(), style=style)
            .permute(0, 2, 1)
            .contiguous()
        )  # BND
        ...
        pt_sigma_1d = (
            output[:, :, self.input_dim : 2 * self.input_dim].contiguous()
            - self.pts_sigma_offset
        )
...
class LatentPointDecPVC(nn.Module):
    ...
    def forward(self, x, beta, context, style):
        ...
        output = (
            self.layers(context.permute(0, 2, 1).contiguous(), style=style)
            .permute(0, 2, 1)
            .contiguous()
        )  # BN3

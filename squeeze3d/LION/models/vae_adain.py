...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.style_encoder = import_model(args.latent_pts.style_encoder)(
            zdim=args.latent_pts.style_dim, input_dim=self.input_dim, args=args
        )
        ...
        self.encoder = import_model(args.shapelatent.encoder_type)(
            zdim=latent_dim, input_dim=self.input_dim, args=args
        )

        # build decoder
        self.decoder = import_model(args.shapelatent.decoder_type)(
            context_dim=latent_dim, point_dim=args.ddpm.input_dim, args=args
        )
        logger.info(
            "[Build Model] style_encoder: {}, encoder: {}, decoder: {}",
    ...
    def encode(self, x, class_label=None):
        ...
        assert x.shape[2] == self.input_dim, (
            f"expect input in " f"[B,Npoint,PointDim={self.input_dim}], get: {x.shape}"
        )
        x_0_target = x
        latent_list = []
        all_eps = []
    ...
    def recont(self, x, target=None, class_label=None, cls_emb=None):
        ...
        # ---- original encoder ---- #
        style = (
            torch.cat([z_global, cls_emb], dim=1)
            if self.args.data.cond_on_cat
            else z_global
        )
        style = self.style_mlp(style) if self.style_mlp is not None else style
        ...
        # ---- decoder ---- #
        x_0_pred = self.decoder(
            None, beta=None, context=z_local, style=style
        )  # (B,ncenter,3)

        make_4d = lambda x: (
            x.unsqueeze(-1).unsqueeze(-1) if len(x.shape) == 2 else x.unsqueeze(-1)
        )
        ...
        if "LatentPoint" in self.args.shapelatent.decoder_type:
            ...
            if "Hir" in self.args.shapelatent.decoder_type:
                latent_pts = (
                    z_local[:, : -self.args.latent_pts.latent_dim_ext[0]]
                    .view(*latent_shape)[:, :, :3]
                    .contiguous()
                    .clone()
                )
                ...
                latent_pts = (
                    z_local.view(*latent_shape)[:, :, : self.input_dim]
                    .contiguous()
                    .clone()
                )

            output["vis/latent_pts"] = (
                latent_pts.detach().cpu().view(batch_size, -1, self.input_dim)
            )  # B,N,3
    ...
    def get_loss(
        ...
        **kwargs,
        ...
        loss_0 = loss_fn(
            x_0_pred, x_0_target, self.args.ddpm.loss_type, self.input_dim, batch_size
        ).mean()
        rec_loss = loss_0
        ...
        for pairs_id, pairs in enumerate(output["latent_list"]):
            ...
            log_sigma = csigma
            kl_term_close = (
                0.5 * log_sigma.exp() ** 2 + 0.5 * cmu**2 - log_sigma - 0.5
            ).view(batch_size, -1)
            if (
                "LatentPoint" in self.args.shapelatent.decoder_type
                and "Hir" not in self.args.shapelatent.decoder_type
            ):
                if pairs_id == 1:
                    latent_shape = [batch_size, -1, self.latent_dim + self.input_dim]
                    kl_pt = kl_term_close.view(*latent_shape)[:, :, : self.input_dim]
                    kl_feat = kl_term_close.view(*latent_shape)[:, :, self.input_dim :]
                    weighted_kl_terms.append(
                        kl_pt.sum(2).sum(1) * self.args.latent_pts.weight_kl_pt
                    )
                    weighted_kl_terms.append(
                        kl_feat.sum(2).sum(1) * self.args.latent_pts.weight_kl_feat
                    )

                    output["print/kl_pt%d" % pairs_id] = kl_pt.sum(2).sum(1)
                    output["print/kl_feat%d" % pairs_id] = kl_feat.sum(2).sum(1)

                    output["print/z_var_pt%d" % pairs_id] = (
                        log_sigma.view(*latent_shape)[:, :, : self.input_dim]
                    ).exp() ** 2
                    output["print/z_var_feat%d" % pairs_id] = (
                        log_sigma.view(*latent_shape)[:, :, self.input_dim :]
                    ).exp() ** 2
                    output["print/z_mean_feat%d" % pairs_id] = cmu.view(*latent_shape)[
                        :, :, self.input_dim :
                    ].mean()
                elif pairs_id == 0:
                    kl_style = kl_term_close
                    weighted_kl_terms.append(
                        kl_style.sum(-1) * self.args.latent_pts.weight_kl_glb
                    )
    ...
    def sample(
        ...
        if "LatentPoint" in self.args.shapelatent.decoder_type:
            ...
            latent_shape = (
                num_samples,
                self.num_points * (self.latent_dim + self.input_dim),
            )
        ...
        if len(decomposed_eps) == 0:
            z_local = torch.zeros(*latent_shape).to(torch.device(device_str)).normal_()
            z_global = (
                torch.zeros(*style_latent_shape).to(torch.device(device_str)).normal_()
            )
        else:
        ...
        style = self.style_mlp(style) if self.style_mlp is not None else style
        x_0_pred = self.decoder(
            None, beta=None, context=z_local, style=z_global
        )  # (B,ncenter,3)

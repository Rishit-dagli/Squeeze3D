...
class PVCNN2Prior(PVCNN2Unet):
    ...
    def __init__(self, args, num_input_channels, cfg):
        ...
        num_input_channels = num_classes = (
            cfg.shapelatent.latent_dim + cfg.ddpm.input_dim
        )
        ...
        logger.info(
            "[Build Prior Model] nclass={}, embed_dim={}, use_att={},"
        ...
        super().__init__(
            ...
            clip_forge_enable=self.clip_forge_enable,
            clip_forge_dim=clip_forge_dim,
        )
        # init mixing logit
        self.mixed_prediction = (
            cfg.sde.mixed_prediction
        )  # This enables mixed prediction
        if self.mixed_prediction:
            logger.info(
                "init-mixing_logit = {}, after sigmoid = {}",
                cfg.sde.mixing_logit_init,
                torch.sigmoid(torch.tensor(cfg.sde.mixing_logit_init)),
            )
            init = cfg.sde.mixing_logit_init * torch.ones(
                size=[1, num_input_channels * self.num_points, 1, 1]
            )
    ...
    def forward(self, x, t, *args, **kwargs):  # x0=None):
        ...
        out = super().forward(
            x,
            t=t,
            style=kwargs["condition_input"].squeeze(-1).squeeze(-1),

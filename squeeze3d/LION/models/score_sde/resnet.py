...
class Prior(nn.Module):
    ...
    def __init__(self, args, num_input_channels, *oargs, **kwargs):
        ...
        self.cfg = oargs[0]
        self.clip_forge_enable = (
            self.cfg.clipforge.enable
        )  # kwargs.get('clipforge.enable', 0)

        logger.info(
            "[Build Resnet Prior] Has condition input: {}; clipforge {}; "
            "learn_mixing_logit={}, ",
            self.condition_input,
            self.clip_forge_enable,
            args.learn_mixing_logit,
        )
        ...
        num_cell_per_scale_dae = (
            args.num_cell_per_scale_dae
            if "num_cell_per_scale_dae" not in kwargs
            else kwargs["num_cell_per_scale_dae"]
        )
        ...
        if self.mixed_prediction:
            logger.info(
                "init-mixing_logit = {}, after sigmoid = {}",
                args.mixing_logit_init,
                torch.sigmoid(torch.tensor(args.mixing_logit_init)),
            )
            ...
            # else:
            if True:
                init = args.mixing_logit_init * torch.ones(
                    size=[1, num_input_channels, 1, 1]
                )
        ...
        self.temb_fun = init_temb_fun(
            args.embedding_type, args.embedding_scale, args.embedding_dim
        )
        logger.info(
            "[temb_fun] embedding_type={}, embedding_scale={}, embedding_dim={}",
            args.embedding_type,
    ...
    def forward(self, x, t, **kwargs):
        ...
        if self.clip_forge_enable:
            ...
            clip_feat = self.clip_feat_mapping(clip_feat[:, :, None])[
                :, :, :, None
            ]  # B,D -> BD1->B,D,1,1

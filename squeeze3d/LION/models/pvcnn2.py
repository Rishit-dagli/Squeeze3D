...
class PVConv(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if coords_input.shape[1] > 3:
            coords = coords_input[
                :, :3
            ]  # the last 3 dim are other point attributes if any
        else:
            coords = coords_input
        assert (
        ...
        if time_emb is None:
            time_emb = {
                "voxel_features_4d": voxel_features_4d,
...
class PointNetAModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if len(self.mlps) > 1:
            ...
            return (
                self.mlps[0](features).max(dim=-1, keepdim=True).values,
                coords,
                time_emb,
            )
...
class PointNetSAModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for grouper, mlp in zip(self.groupers, self.mlps):
            ...
            features_list.append(mlp(grouper_output).max(dim=-1).values)
        ...
        if conv_configs is not None:
            ...
            for p in range(num_blocks):
                ...
                if c == 0:
                    ...
                    sa_blocks.append(
                        block(in_channels + embed_dim * has_temb, out_channels)
                    )
                in_channels = out_channels
                k += 1

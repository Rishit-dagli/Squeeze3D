...
from .tsal_base import Latent2MeshOutput, Point2MeshOutput, ShapeAsLatentModule
...
class ShapeAsLatentPLModule(pl.LightningModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.sal: ShapeAsLatentModule = instantiate_from_config(
            module_cfg, device=None, dtype=None
        )
    ...
    def configure_optimizers(self) -> Tuple[List, List]:
        ...
        if self.optimizer_cfg is None:
            optimizers = [
                torch.optim.AdamW(
                    self.sal.parameters(), lr=lr, betas=(0.9, 0.99), weight_decay=1e-3
                )
            ]
            schedulers = []
        else:
            optimizer = instantiate_from_config(
                self.optimizer_cfg.optimizer, params=self.sal.parameters()
            )
    ...
    def decode(
        ...
        outputs = self.latent2mesh(
            latents, bounds=bounds, octree_depth=octree_depth, num_chunks=num_chunks
        )
    ...
    def training_step(
        ...
        posterior, logits = self(pc=pc, feats=feats, volume_queries=volume_queries)
    ...
    def point2mesh(
        ...
        pc: torch.FloatTensor,
        feats: torch.FloatTensor,
        bounds: Union[Tuple[float], List[float]] = (
            -1.25,
            -1.25,
            -1.25,
            1.25,

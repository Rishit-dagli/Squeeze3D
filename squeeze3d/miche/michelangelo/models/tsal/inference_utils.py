...
from squeeze3d.miche.michelangelo.graphics.primitives import (
...
def extract_geometry(
    ...
    xyz_samples, grid_size, length = generate_dense_grid_points(
        bbox_min=bbox_min, bbox_max=bbox_max, octree_depth=octree_depth, indexing="ij"
    )
    ...
    for i in range(batch_size):
        try:
            vertices, faces, normals, _ = measure.marching_cubes(
                grid_logits[i], 0, method="lewiner"
            )

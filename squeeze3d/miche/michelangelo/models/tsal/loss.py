...
from squeeze3d.miche.michelangelo.models.modules.distributions import (
    DiagonalGaussianDistribution,
)
from squeeze3d.miche.michelangelo.utils import misc
from squeeze3d.miche.michelangelo.utils.eval import compute_psnr
...
class KLNearFarColor(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if posteriors is None:
            kl_loss = torch.tensor(
                0.0, dtype=pred_colors.dtype, device=pred_colors.device
            )
        else:
            kl_loss = posteriors.kl(dims=(1, 2))
            kl_loss = torch.mean(kl_loss)
        ...
        **kwargs,
        ...
        if local_batch_size != self.last_local_batch_size:
            self.labels = (
                ...
                + torch.arange(local_batch_size, device=shape_embed.device).long()
            )
        ...
        contrast_loss = (
            F.cross_entropy(logits_per_shape_text, self.labels)
            + F.cross_entropy(logits_per_text_shape, self.labels)
        ) / 2 + (
            F.cross_entropy(logits_per_shape_image, self.labels)
            + F.cross_entropy(logits_per_image_shape, self.labels)
        ) / 2

...
from squeeze3d.miche.michelangelo.data.transforms import RandomResize
...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    ...
    def __init__(
        ...
        self.transform = transforms.Compose(
            [
                transforms.Resize(
                    224, transforms.InterpolationMode.BICUBIC, antialias=True
                ),
    ...
    def unconditional_embedding(self, batch_size):
        ...
        if self.num_projection_vector > 0:
            zero = self.projection(zero).view(
                batch_size, self.num_projection_vector, -1
            )
        return zero
    ...
    def forward(self, image, value_range=(-1, 1), zero_embedding_radio=0):
        ...
        if zero_embedding_radio > 0:
            mask = (
                torch.rand((len(image), 1, 1), device=z.device, dtype=z.dtype)
                < zero_embedding_radio
            )
            z = z * mask.to(z)
...
class FrozenCLIPImageGridEmbedder(AbstractEncoder):
    ...
    def __init__(
        ...
        self.transform = transforms.Compose(
            [
                transforms.Resize(
                    224, transforms.InterpolationMode.BILINEAR, antialias=True
                ),
                transforms.CenterCrop(224),  # crop a (224, 224) square
                transforms.Normalize(
                    mean=[0.48145466, 0.4578275, 0.40821073],
...
class MoECLIPImageEncoder(nn.Module):
    def __init__(
        ...
        if self.num_projection_vector == 0:
            ...
            self.projection = nn.Linear(
                ...
                hidden_state_dim * self.num_projection_vector,
                bias=True,
            )
    ...
    def convert_embedding(self, z):
        if self.num_projection_vector > 0:
            z = self.projection(z.type(self.projection.weight.dtype)).view(
                len(z), self.num_projection_vector, -1
            )
        return z

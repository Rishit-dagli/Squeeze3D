...
from squeeze3d.miche.michelangelo.models.asl_diffusion.inference_utils import (
    ddim_sample,
)

# from squeeze3d.miche.michelangelo.models.tsal.tsal_base import ShapeAsLatentPLModule
from squeeze3d.miche.michelangelo.models.tsal.tsal_base import (
    AlignedShapeAsLatentPLModule,
)
from squeeze3d.miche.michelangelo.utils import instantiate_from_config
...
class ASLDiffuser(pl.LightningModule):
    ...
    def __init__(
        ...
        *,
        ...
        self.noise_scheduler: DDPMScheduler = instantiate_from_config(
            scheduler_cfg.noise
        )
        self.denoise_scheduler: SchedulerType = instantiate_from_config(
            scheduler_cfg.denoise
        )
    ...
    def encode_surface(self, surface):
        ...
        return self.first_stage_model.model.encode_shape_embed(
            surface, return_latents=False
        )
    ...
    def decode_first_stage(self, z_q: torch.FloatTensor, **kwargs):
        ...
        z_q = 1.0 / self.z_scale_factor * z_q
    ...
    def compute_loss(self, model_outputs, split):
        ...
        if self.noise_scheduler.prediction_type == "epsilon":
            ...
            raise NotImplementedError(
                f"Prediction Type: {self.noise_scheduler.prediction_type} not yet supported."
            )
        ...
        if self.loss_cfg.loss_type == "l1":
            ...
            raise NotImplementedError(
                f"Loss Type: {self.loss_cfg.loss_type} not yet supported."
            )
    ...
    def forward(self, batch):
        ...
        conditions = self.cond_stage_model[self.cond_stage_key](
        ...
        diffusion_outputs = {"x_0": noisy_z, "noise": noise, "pred": noise_pred}
    ...
    def training_step(
        ...
        self.log_dict(
            loss_dict, prog_bar=True, logger=True, sync_dist=False, rank_zero_only=True
        )
    ...
    def sample(
        ...
        **kwargs,
        ...
        if do_classifier_free_guidance:
            ...
            un_cond = self.cond_stage_model[
                f"{self.cond_stage_key}_unconditional_embedding"
            ](cond)

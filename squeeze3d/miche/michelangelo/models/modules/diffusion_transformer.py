...
from squeeze3d.miche.michelangelo.models.modules.checkpoint import checkpoint
...
class AdaLayerNorm(nn.Module):
    def __init__(self, device: torch.device, dtype: torch.dtype, width: int):
...
class DitBlock(nn.Module):
    ...
    def forward(
        ...
        return checkpoint(
            self._forward, (x, t, context), self.parameters(), self.use_checkpoint
        )
    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(layers):
            ...
            layer_norm = (
                nn.LayerNorm(width, device=device, dtype=dtype) if skip_ln else None
            )

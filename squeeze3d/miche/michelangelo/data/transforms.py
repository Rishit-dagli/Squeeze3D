...
class RandomSample(object):
    def __init__(self, num_volume_samples: int = 1024, num_near_samples: int = 1024):
    ...
    def __call__(self, sample):
        ...
        vol_points_labels = np.concatenate(
            [vol_points, vol_label[:, np.newaxis]], axis=1
        )

        ind = rng.choice(near_points.shape[0], self.num_near_samples, replace=False)
        near_points = near_points[ind]
        near_label = near_label[ind]
        near_points_labels = np.concatenate(
            [near_points, near_label[:, np.newaxis]], axis=1
        )

        # concat sampled volume and near points
        geo_points = np.concatenate([vol_points_labels, near_points_labels], axis=0)

        sample = {"surface": surface, "geo_points": geo_points}
...
class SplitRandomSample(object):
    ...
    def __call__(self, sample):
        ...
        if self.use_surface_sample:
            ...
            ind = rng.choice(
                surface.shape[0], self.num_surface_samples, replace=replace
            )
            surface = surface[ind]
...
class RandomResize(torch.nn.Module):
    ...
    def random_resize_params(self):
        radio = (
            torch.rand(1) * (self.resize_radio[1] - self.resize_radio[0])
            + self.resize_radio[0]
        )

...
from .color_util import gen_checkers, gen_circle, get_colors
...
class PyThreeJSViewer(object):
    ...
    def __init__(self, settings, render_mode="WEBSITE"):
        ...
        self._light = p3s.DirectionalLight(
        ...
        self._scene = p3s.Scene(
            children=[self._cam, self._light2], background=self.__s["background"]
        )  # "#4c4c80"
    ...
    def __get_colors(self, v, f, c, sh):
        ...
        if type(c) == np.ndarray and c.size == 3:  # Single color
            ...
            type(c) == np.ndarray and len(c.shape) == 2 and c.shape[1] == 3
        ):  # Color values for
            ...
            type(c) == np.ndarray and c.size == f.shape[0]
        ):  # Function values for faces
            ...
            type(c) == np.ndarray and c.size == v.shape[0]
        ):  # Function values for vertices
            normalize = sh["normalize"][0] != None and sh["normalize"][1] != None
            colors = get_colors(
                c,
                sh["colormap"],
    ...
    def add_mesh(
        ...
        if uv is not None:
            ...
            tex = p3s.DataTexture(
                data=texture_data, format="RGBFormat", type="FloatType"
            )
        ...
        if (
            type(n) != type(None) and coloring == "VertexColors"
        ):  # TODO: properly handle normals for FaceColors as well
            ba_dict["normal"] = p3s.BufferAttribute(
                n.astype("float32", copy=False), normalized=True
            )
        ...
        if sh["wireframe"]:
            ...
            wf_material = p3s.LineBasicMaterial(
                color=sh["wire_color"], linewidth=sh["wire_width"]
            )
        ...
        mesh_obj["arrays"] = [
            ...
            f,
            c,
        ]  # TODO replays with proper storage or remove if not needed
    ...
    def add_lines(self, beginning, ending, shading={}, obj=None, **kwargs):
        ...
        if len(beginning.shape) == 1:
            ...
            if beginning.shape[1] == 2:
                beginning = np.append(beginning, np.zeros([beginning.shape[0], 1]), 1)
        if len(ending.shape) == 1:
            if len(ending) == 2:
                ending = np.array([[ending[0], ending[1], 0]])
        else:
            if ending.shape[1] == 2:
                ending = np.append(ending, np.zeros([ending.shape[0], 1]), 1)
    ...
    def add_edges(self, vertices, edges, shading={}, obj=None, **kwargs):
        ...
        if vertices.shape[1] == 2:
            vertices = np.append(vertices, np.zeros([vertices.shape[0], 1]), 1)
        sh = self.__get_shading(shading)
    ...
    def add_points(self, points, c=None, shading={}, obj=None, **kwargs):
        ...
        if sh["point_shape"] == "circle":  # Plot circles
            tex = p3s.DataTexture(
                data=gen_circle(16, 16), format="RGBAFormat", type="FloatType"
            )
    ...
    def update_object(self, oid=0, vertices=None, colors=None, faces=None):
        ...
        if type(colors) != type(None):
            colors, coloring = self.__get_colors(
                obj["arrays"][0], obj["arrays"][1], colors, obj["shading"]
            )
            colors = colors.astype("float32", copy=False)
    ...
    def to_html(self, imports=True, html_frame=True):
        ...
        for r, obj in enumerate(self.__objects):
            ...
            v -= mean
            v += np.array([0.0, 0.9, 0.0])  #! to move the obj to the center of window
        ...
        self._cam.position = [
            ...
            0.5,
            scale * 1.3,
        ]  #! show four complete meshes in the window

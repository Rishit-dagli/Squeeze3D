...
from .swin_mae3d import SwinTransformer_MAE3D
...
def main():
    ...
    grid_vis_original_path = os.path.join(
        output_folder, filename + "_grid_vis_original.npy"
    )
    np.save(grid_vis_original_path, grid_vis_original)

    target_rgb_vis_original_path = os.path.join(
        output_folder, filename + "_target_rgb_vis_original.npy"
    )
    ...
    target_rgb_vis_patches = target_rgb.reshape(
        -1, 3
    ) * mask_remove_patch_unroll.reshape(-1, 1)
    # grid_vis_patches = grid_vis_patches[viz_mask, :]
    # target_rgb_vis_patches = target_rgb_vis_patches[viz_mask, :]

    grid_vis_patches_path = os.path.join(
        output_folder, filename + "_grid_vis_patches.npy"
    )
    np.save(grid_vis_patches_path, grid_vis_patches)

    target_rgb_vis_patches_path = os.path.join(
        output_folder, filename + "_target_rgb_vis_patches.npy"
    )

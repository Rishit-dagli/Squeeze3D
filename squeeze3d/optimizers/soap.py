...
def get_orthogonal_matrix_QR(
    GG, Q, exp_avg_sq, max_precond_dim=10000, merge_dims=False
):
    """
    ...
    indices = tuple(
        (
            slice(None)
            if ind is None
            else ind.view(*(1,) * i, -1, *(1,) * (exp_avg_sq_new.dim() - i - 1))
        )  #
...
def update_preconditioner(
    grad, state, max_precond_dim, merge_dims, precondition_1d, beta, update_precond
):
    """
    ...
    if update_precond:
        get_orthogonal_matrix_QR(
            state["GG"], state["Q"], state["exp_avg_sq"], max_precond_dim, merge_dims
        )


def init_preconditioner(
    grad, state, max_precond_dim=10000, precondition_1d=False, merge_dims=False
):
    """
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if "exp_avg" not in state:
                    ...
                    init_preconditioner(
                        grad, state, max_precond_dim, precondition_1d, merge_dims
                    )
                ...
                grad_projected = project(
                    grad, state["Q"], merge_dims, max_precond_dim, False
                )
            ...
            for p, g, ea, d in zip(p_list, grad, exp_avg, denom):
                ...
                # i.e. projecting to the eigenbases of matrices in state['GG']
                exp_avg_projected = project(
                    ea, state["Q"], merge_dims, max_precond_dim, False
                )

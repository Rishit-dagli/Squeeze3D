...
def get_spherical(T_target, T_cond):
    ...
    d_T = torch.tensor(
        [math.degrees(d_theta.item()), math.degrees(d_azimuth.item()), d_z.item()]
    )
    return d_T
...
class RTMV(Dataset):
    def __init__(
        self,
        root_dir="datasets/RTMV/google_scanned",
        first_K=64,
        resolution=256,
        load_target=False,
    ):
    ...
    def __getitem__(self, idx):
        ...
        imgs = (np.array(imgs) / 255.0).astype(np.float32)  # (RGBA) imgs
        imgs = torch.tensor(self.blend_rgba(imgs)).permute(0, 3, 1, 2)
        imgs = imgs * 2 - 1.0  # convert to stable diffusion range
        poses = torch.tensor(np.array(poses).astype(np.float32))
        return imgs, poses

    def blend_rgba(self, img):
        img = img[..., :3] * img[..., -1:] + (1.0 - img[..., -1:])  # blend A to RGB
        return img
...
class GSO(Dataset):
    def __init__(
        self,
        root_dir="datasets/GoogleScannedObjects",
        split="val",
        first_K=5,
        resolution=256,
        load_target=False,
...
class WILD(Dataset):
    def __init__(
        self, root_dir="data/nerf_wild", first_K=33, resolution=256, load_target=False
    ):
        self.root_dir = root_dir
        self.scene_list = sorted(next(os.walk(root_dir))[1])
        self.resolution = resolution

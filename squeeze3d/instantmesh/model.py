...
class MVRecon(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 1000 == 0 and self.global_rank == 0:
            ...
            input_images = v2.functional.resize(
                lrm_generator_input["images"], (H, W), interpolation=3, antialias=True
            ).clamp(0, 1)
            input_images = torch.cat(
                [input_images, torch.ones(B, N - N_in, C, H, W).to(input_images)], dim=1
            )

            input_images = rearrange(input_images, "b n c h w -> b c h (n w)")
            ...
            render_alphas = rearrange(
                repeat(render_out["render_alphas"], "b n 1 h w -> b n 3 h w"),
                "b n c h w -> b c h (n w)",
            )
            ...
            render_depths = rearrange(
                repeat(render_out["render_depths"], "b n 1 h w -> b n 3 h w"),
                "b n c h w -> b c h (n w)",
            )
    ...
    def configure_optimizers(self):
        ...
        params.append(
            {"params": self.lrm_generator.parameters(), "lr": lr, "weight_decay": 0.01}
        )

        optimizer = torch.optim.AdamW(params, lr=lr, betas=(0.90, 0.95))
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, 3000, eta_min=lr / 10
        )

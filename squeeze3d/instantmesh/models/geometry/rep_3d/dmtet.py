...
def marching_tets(
    ...
    with torch.no_grad():
        ...
        mapping = (
            torch.ones((unique_edges.shape[0]), dtype=torch.long, device=sdf_n.device)
            * -1
        )
...
def marching_tets_tetmesh(
    ...
    tets = torch.cat(
        (
            torch.gather(
                input=idx_map[num_tets == 1],
                dim=1,
                index=tet_table[tetindex[num_tets == 1]][:, :4],
            ).reshape(-1, 4),
            torch.gather(
                input=idx_map[num_tets == 3],
                dim=1,
                index=tet_table[tetindex[num_tets == 3]][:, :12],
            ).reshape(-1, 4),
        ),
        dim=0,
    )
...
def batch_subdivide_volume(tet_pos_bxnx3, tet_bxfx4, grid_sdf):
    ...
    mid_points_pos = (
        all_values[:, unique_edges.reshape(-1)]
        .reshape(all_values.shape[0], -1, 2, all_values.shape[-1])
        .mean(2)
    )
...
def tet_to_tet_adj_sparse(tet_tx4):
    ...
    with torch.no_grad():
        ...
        idx_array = (
            torch.LongTensor([0, 1, 2, 1, 0, 3, 2, 3, 0, 3, 2, 1])
            .to(device)
            .reshape(4, 3)
            .unsqueeze(0)
            .expand(t, -1, -1)
        ...
        # get all faces
        all_faces = torch.gather(
            input=tet_tx4.unsqueeze(1).expand(-1, 4, -1), index=idx_array, dim=-1
        ).reshape(
            -1, 3
        )  # (tx4, 3)
        all_faces_tet_idx = (
            torch.arange(t, device=device).unsqueeze(-1).expand(-1, 4).reshape(-1)
        )
        # sort and group
        all_faces_sorted, _ = torch.sort(all_faces, dim=1)

        all_faces_unique, inverse_indices, counts = torch.unique(
            all_faces_sorted, dim=0, return_counts=True, return_inverse=True
        )
        ...
        tet_face_tetidx_fx2 = torch.stack(
            [all_faces_tet_idx_grouped[::2], all_faces_tet_idx_grouped[1::2]], dim=-1
        )

        tet_adj_idx = torch.cat(
            [tet_face_tetidx_fx2, torch.flip(tet_face_tetidx_fx2, [1])]
        )
        ...
        values = torch.ones(tet_adj_idx.shape[0], device=tet_tx4.device).float()
        adj_sparse = torch.sparse.FloatTensor(
            tet_adj_idx.t(), values, torch.Size([t, t])
        )

        # normalization
        neighbor_num = 1.0 / torch.sparse.sum(adj_sparse, dim=1).to_dense()
...
def get_tet_bxfx4x3(bxnxz, bxfx4):
    ...
    gather_input = bxnxz.unsqueeze(2).expand(n_batch, bxnxz.shape[1], 4, z)
    gather_index = bxfx4.unsqueeze(-1).expand(n_batch, bxfx4.shape[1], 4, z).long()
    tet_bxfx4xz = torch.gather(input=gather_input, dim=1, index=gather_index)
...
def shrink_grid(tet_pos_bxnx3, tet_bxfx4, grid_sdf):
    with torch.no_grad():
        ...
        occ = grid_sdf[0] > 0
        occ_sum = (
            get_tet_bxfx4x3(occ.unsqueeze(0).unsqueeze(-1), tet_bxfx4)
            .reshape(-1, 4)
            .sum(-1)
        )
        ...
        mapping = torch.zeros(
            (tet_pos_bxnx3.shape[1]), device=tet_pos_bxnx3.device, dtype=torch.long
        )
        ...
        mapping[selected_verts_idx] = torch.arange(
            selected_verts_idx.shape[0], device=tet_pos_bxnx3.device
        )
...
class DMTetGeometry(Geometry):
    def __init__(
        ...
        (
            self.triangle_table,
            self.num_triangles_table,
            self.base_tet_edges,
            self.v_id,
        ) = create_mt_variable(self.device)
    ...
    def get_tet_mesh(self, v_deformed_nx3, sdf_n, with_uv=False, indices=None):
        ...
        verts, faces, tet_verts, tets = marching_tets_tetmesh(
            ...
            self.base_tet_edges,
            self.v_id,
            return_tet_mesh=True,
            num_tets_table=self.num_tets_table,
            tet_table=self.tet_table,
    ...
    def render_mesh(
        ...
        if self.render_type == "neural_render":
            tex_pos, mask, hard_mask, rast, v_pos_clip, mask_pyramid, depth = (

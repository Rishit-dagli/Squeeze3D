...
class MipRayMarcher2(nn.Module):
    ...
    def run_forward(self, colors, densities, depths, rendering_options, normals=None):
        ...
        densities_mid = self.activation_factory(rendering_options)(densities_mid).to(
            dtype
        )
    ...
    def forward(self, colors, densities, depths, rendering_options, normals=None):
        if normals is not None:
            composite_rgb, composite_depth, composite_normals, weights = (
                self.run_forward(colors, densities, depths, rendering_options, normals)
            )

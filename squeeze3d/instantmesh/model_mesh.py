...
class MVRecon(pl.LightningModule):
        ...
        if init_ckpt is not None:
            ...
            for k, v in sd.items():
                if k.startswith("lrm_generator.synthesizer.decoder.net."):
                    if k.startswith(
                        "lrm_generator.synthesizer.decoder.net.6."
                    ):  # last layer
    ...
    def prepare_batch_data(self, batch):
        ...
        images = v2.functional.resize(
            images, self.input_size, interpolation=3, antialias=True
        ).clamp(0, 1)
        ...
        target_images = torch.cat(
            [batch["input_images"], batch["target_images"]], dim=1
        )
        target_depths = torch.cat(
            [batch["input_depths"], batch["target_depths"]], dim=1
        )
        target_alphas = torch.cat(
            [batch["input_alphas"], batch["target_alphas"]], dim=1
        )
        target_normals = torch.cat(
            [batch["input_normals"], batch["target_normals"]], dim=1
        )

        render_size = self.render_size
        target_images = v2.functional.resize(
            target_images, render_size, interpolation=3, antialias=True
        ).clamp(0, 1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.global_step % 1000 == 0 and self.global_rank == 0:
            ...
            target_alphas = rearrange(
                repeat(render_gt["target_alphas"], "b n 1 h w -> b n 3 h w"),
                "b n c h w -> b c h (n w)",
            )
            render_alphas = rearrange(
                repeat(render_out["mask"], "b n 1 h w -> b n 3 h w"),
                "b n c h w -> b c h (n w)",
            )
            target_depths = rearrange(
                repeat(render_gt["target_depths"], "b n 1 h w -> b n 3 h w"),
                "b n c h w -> b c h (n w)",
            )
            render_depths = rearrange(
                repeat(render_out["depth"], "b n 1 h w -> b n 3 h w"),
                "b n c h w -> b c h (n w)",
            )
            target_normals = rearrange(
                render_gt["target_normals"], "b n c h w -> b c h (n w)"
            )
            render_normals = rearrange(render_out["normal"], "b n c h w -> b c h (n w)")
            ...
            grid = make_grid(
                grid, nrow=target_images.shape[0], normalize=True, value_range=(0, 1)
            )
    ...
    def compute_loss(self, render_out, render_gt):
        ...
        loss_depth = 0.5 * F.l1_loss(
            render_depths[target_alphas > 0], target_depths[target_alphas > 0]
        )
        ...
        sdf_reg_loss_entropy = (
            sdf_reg_loss_batch(sdf, self.lrm_generator.geometry.all_edges).mean() * 0.01
        )
        ...
        loss_reg = (
            sdf_reg_loss_entropy + flexicubes_surface_reg + flexicubes_weights_reg
        )
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(
            self.lrm_generator.parameters(),
            lr=lr,
            betas=(0.90, 0.95),
            weight_decay=0.01,
        )
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, 100000, eta_min=0
        )

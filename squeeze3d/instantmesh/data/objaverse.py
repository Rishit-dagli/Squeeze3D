...
from squeeze3d.instantmesh.utils.camera_util import (
    FOV_to_intrinsics,
    center_looking_at_camera_pose,
    get_circular_camera_poses,
)
from squeeze3d.instantmesh.utils.train_util import instantiate_from_config
    ...
    def __getitem__(self, index):
        while True:
            input_image_path = os.path.join(
                self.root_dir, self.input_image_dir, self.paths[index]
            )
            target_image_path = os.path.join(
                self.root_dir, self.target_image_dir, self.paths[index]
            )

            indices = np.random.choice(
                range(self.total_view_n),
                self.input_view_num + self.target_view_num,
                replace=False,
            )
            ...
            try:
                input_cameras = np.load(os.path.join(input_image_path, "cameras.npz"))[
                    "cam_poses"
                ]
                for idx in input_indices:
                    image, alpha = self.load_im(
                        os.path.join(input_image_path, "%03d.png" % idx), bg_white
                    )
                    normal, _ = self.load_im(
                        os.path.join(input_image_path, "%03d_normal.png" % idx),
                        bg_black,
                    )
                    depth = (
                        cv2.imread(
                            os.path.join(input_image_path, "%03d_depth.png" % idx),
                ...
                target_cameras = np.load(
                    os.path.join(target_image_path, "cameras.npz")
                )["cam_poses"]
                for idx in target_indices:
                    image, alpha = self.load_im(
                        os.path.join(target_image_path, "%03d.png" % idx), bg_white
                    )
                    normal, _ = self.load_im(
                        os.path.join(target_image_path, "%03d_normal.png" % idx),
                        bg_black,
                    )
                    depth = (
                        cv2.imread(
                            os.path.join(target_image_path, "%03d_depth.png" % idx),
                            cv2.IMREAD_UNCHANGED,
                        )
                        / 255.0
                        * self.depth_scale
        ...
        if self.camera_rotation:
            ...
            normals = normals * 2.0 - 1.0
            normals = torch.matmul(rot[:, :3, :3], normals.view(N, 3, -1)).view(
                N, 3, H, W
            )
            normals = F.normalize(normals, dim=1)
            normals = (normals + 1.0) / 2.0
        ...
        Ks = (
            K.unsqueeze(0)
            .repeat(self.input_view_num + self.target_view_num, 1, 1)
            .float()
        )
...
class ValidationData(Dataset):
    def __init__(
        ...
        render_c2ws = get_circular_camera_poses(
            M=8, radius=cam_distance, elevation=20.0
        )
        render_Ks = (
            FOV_to_intrinsics(self.fov).unsqueeze(0).repeat(render_c2ws.shape[0], 1, 1)
        )
    ...
    def __getitem__(self, index):
        ...
        for idx in range(self.input_view_num):
            image, alpha = self.load_im(
                os.path.join(input_image_path, f"{idx:03d}.png"), bkg_color
            )
            image_list.append(image)
            alpha_list.append(alpha)

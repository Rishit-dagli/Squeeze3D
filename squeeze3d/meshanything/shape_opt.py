...
class ShapeOPT(OPTForCausalLM):
    ...
    @replace_return_docstrings(
        output_type=CausalLMOutputWithPast, config_class="OPTConfig"
    )
    def forward(
        self,
...
class ShapeOPTDecoder(OPTDecoder):
    ...
    def __init__(self, config: ShapeOPTConfig):
        ...
        self.extra_embeds = nn.Embedding(
            3, config.word_embed_proj_dim
        )  # padding_idx=self.padding_idx)
        self.input_layer = nn.Linear(
            config.quantize_codebook_dim, config.word_embed_proj_dim
        )

        self.embed_positions = OPTLearnedPositionalEmbedding(
            config.max_position_embeddings, config.hidden_size
        )
        self.token_embed_positions = OPTFacePositionalEmbedding(
            config.face_per_token + 3, config.word_embed_proj_dim
        )  # padding_idx=self.padding_idx)
    ...
    def embed_with_vae(self, input_ids):
        inputs_embeds = (
            repeat(
                torch.zeros(input_ids.shape, device=input_ids.device),
                "b n -> b n d",
                d=self.word_embed_proj_dim,
            )
            .clone()
            .detach()
        )
        idx_in_extra = torch.isin(
            input_ids, torch.LongTensor([0, 1, 2]).to(input_ids.device)
        )
        ...
        inputs_embeds[~idx_in_extra] += self.input_layer(
            self.quantize_codebooks[0][input_ids[~idx_in_extra] - 3]
        )
    ...
    def forward(
        ...
        if input_ids is not None:
            ...
            inputs_embeds = self.embed_with_vae(
                input_ids
            )  # nothing to do with position

            face_embeds = self.token_embed_positions(
                attention_mask[:, self.cond_length :],
                face_ids,
                input_ids,
                self.face_per_token,
            )
        ...
        batch_size, seq_length = inputs_embeds.shape[
            :2
        ]  # seq_length not used since mask_seq_length is not used
        ...
        )  # not used since attention mask is input
...
class OPTFacePositionalEmbedding(nn.Embedding):
    ...
    def forward(
        self, attention_mask=None, face_ids=None, input_ids=None, face_per_token=None
    ):

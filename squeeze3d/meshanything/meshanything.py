...
class NoiseResistantDecoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.decoder = (
            AutoModel.from_config(config=config).to_bettertransformer().encoder
        )
    ...
    def process_point_feature(self, encode_feature):
        point_feature = torch.zeros(
            encode_feature.shape[0],
            self.cond_length,
            self.n_embd,
        ...
        point_feature = self.point_layernorm(
            point_feature + self.point_pe.weight[None, : point_feature.shape[1]]
        )
    ...
    def forward(self, input_ids, input_embeds, point_feature=None):
        ...
        face_embeds = self.layernorm(
            face_embeds + self.pos_embedding.weight[None, : face_embeds.shape[1]]
        )

        outputs = self.decoder(
            hidden_states=torch.concatenate([point_feature, face_embeds], dim=1),
        )
        decoded = outputs.last_hidden_state[
            :, self.cond_length :
        ]  # batch x nfaces x dim
        decoded = decoded.masked_fill(~face_mask.unsqueeze(-1), 0.0)

        # batch x nfaces x 9 -> batch x nfaces x 3 x 3
        pred_face_logits = self.to_coor_logits(
            decoded
        )  # batch x nfaces x 9 x ndiscrete
        pred_face_coords = rearrange(
            pred_face_logits.argmax(dim=-1), "... (v c) -> ... v c", v=3
        )

        continuous_coors = undiscretize(
            pred_face_coords, num_discrete=self.discrete_num, low=-0.5, high=0.5
        )
        continuous_coors = continuous_coors.masked_fill(
            ~rearrange(face_mask, "b nf -> b nf 1 1"), float("nan")
        )
...
class MeshAnything(nn.Module):
    def __init__(self, args):
        ...
        self.cond_dim = 768
        self.max_length = (
            args.n_max_triangles * self.face_per_token + 2 + self.cond_length
        )
        ...
        self.transformer.model.decoder.quantize_codebooks = nn.Parameter(
            torch.zeros(1, self.tokenizer.codebook_size, self.tokenizer.codebook_dim)
        )
    ...
    def process_point_feature(self, point_feature):
        ...
        point_feature = point_feature.to(device)
    ...
    def forward(self, pc_normal, sampling=False) -> dict:
        ...
        decoder_output = self.tokenizer(
            outputs, code_embed, point_feature=point_feature
        )
...
def undiscretize(t, low, high, num_discrete) -> Tensor:
    t = t.float()
